...
function syncPvDom(){
  ...
  for(const pv of state.previews){
    ...
    if(!el){
      ...
      /* 缩放按钮：用户内容缩放（存 dataset，由 syncPvContentZoom 与画布 zoom 合并生效） */
      let pvZoom=1;
      const zoomVal=el.querySelector(".pv-zoom-val");
      el.dataset.pvZoom="1";
      const applyZoom=()=>{
        el.dataset.pvZoom=String(pvZoom);
...
function ensureMorphDom(it){
  ...
  tools.innerHTML='<span class="pv-morph-name"></span><button class="pv-morph-zo" title="缩小预览">−</button><button class="pv-morph-zoom" title="恢复 100%">100%</button><button class="pv-morph-zi" title="放大预览">+</button><button class="pv-morph-open" title="使用电脑默认应用打开">默认打开</button><button class="pv-morph-close" title="收起预览" aria-label="收起预览">⌃</button>';
  ...
  const applyZoom=value=>{
    ...
    el.dataset.pvZoom=String(zoom);body.style.zoom=f.kind==="link"?"1":String(zoom);
  ...
  /* 预览内容区保留给滚动、翻页、链接等原生操作；卡片的窄边则作为可靠选中区。 */
  el.addEventListener("pointerdown",e=>{
    const r=el.getBoundingClientRect();
    const edge=8;
    const onEdge=e.clientX-r.left<=edge||r.right-e.clientX<=edge||e.clientY-r.top<=edge||r.bottom-e.clientY<=edge;
    if(!onEdge)return;
    e.preventDefault();e.stopPropagation();
    if(state.selected!==it.id||state.multiSel.length!==1||state.multiSel[0]!==it.id){
...
function openFullscreen(fileIdOrItem,options={}){
  ...
  fv.hidden=false;
  layoutFullscreen();
  if(fullscreenToken===myToken)requestAnimationFrame(()=>{fv.classList.add("open");});
  const render=()=>{
...
function closeFullscreen(){
  ...
  const fv=document.getElementById("fullscreenView");
  if(fv){
    fv.classList.remove("open");fv.classList.add("closing");
    const finish=()=>{
      fv.hidden=true;fv.classList.remove("closing","editor-view");
...
function kickMorph(el,url,type){
  ...
  if(type==="img"){
    ...
    iframe.style.cssText="display:block;position:absolute;inset:0;border:none;width:100%;height:100%;min-height:0;flex:none;background:#fff";
...
function fitDocxPreview(bodyEl){
  /* 仅画布中的小预览等比缩放；全屏阅读仍保留 Word 原始分页尺度。
     注意：不能使用 zoom，它会让浏览器先按卡片宽度重排 Word，再缩放，
     结果就是一行文字被压成十来个字。这里缩放的是整张原始 Word 页面。 */
  ...
  /* 用 Word 自己的纸张宽度作为量尺，而不是外层 wrapper（它可能铺满卡片）。 */
  ...
  /* C7 fix: account for body zoom — when content is zoomed, stage width must compensate */
  const contentBody=bodyEl.querySelector(".pv-morph-content");
  const bodyZoom=parseFloat(contentBody&&contentBody.style.zoom)||1;
  const stageWidth=Math.max(120,(bodyEl.clientWidth||0)/bodyZoom);
  const available=Math.max(120,stageWidth-16);
  const scale=clamp(available/nativeWidth,.12,3);
  const scaledW=nativeWidth*scale;
  stage.style.width=(scaledW+16)+"px";
...
async function renderSheet(b,bodyEl){
  ...
  try{
    ...
    bodyEl.querySelectorAll("table").forEach(t=>{t.style.cssText="border-collapse:collapse;margin:10px 0;font-size:12px;min-width:100%;background:#fff";t.querySelectorAll("td,th").forEach(c=>{c.style.cssText="border:1px solid rgba(0,0,0,.09);padding:5px 9px;font-weight:400;white-space:pre-wrap;vertical-align:top";});});
  }catch(e){
