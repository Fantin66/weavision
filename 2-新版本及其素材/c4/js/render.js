...
function render(){
  ...
  syncDetailReadDom(); /* 展开内容与编辑器使用同一 Markdown DOM 预览器 */
...
function syncPvContentZoom(el,pv,z){
  ...
  if(!body)return;
  /* 1) 网页链接（iframe）：
        ★ 复用 NoTab 方案：iframe 100% 撑满容器、流式排版、原生滚动 —— 内容完整显示不裁剪
        ★ 画布缩放同步：用 CSS zoom（布局级缩放，不裁剪、不破坏滚动，
            内容真正等比放大/缩小，与画布其余元素联动） ★
        zoom 基准 = 画布 zoom × 用户 pvZoom；手机模式固定 375 宽视口，
        在 100% 撑满基础上额外用 zoom 等比（375/容器宽 × z）使网页缩放同步。 */
  ...
  if(iframe&&stage){
    ...
    if(mb){
      /* 手机模式：视口固定 375 逻辑宽，内容按 375/可用宽 × z 等比缩放 */
      const availW=Math.max(60,pv.w-16);
      const ratio=(375/availW)*userZoom;
      iframe.style.width="375px";
      iframe.style.height="100%";
      applyEmbeddedPageZoom(iframe,ratio);
      /* 舞台：中心 + 横向可滚，保证 375 视口外内容可完整查看 */
      ...
      /* PC 模式：iframe 100% 撑满可用内容区；仅响应用户主动缩放。 */
      iframe.style.width="100%";
      iframe.style.height="100%";
      applyEmbeddedPageZoom(iframe,userZoom);
...
function syncMorphDom(){
  ...
  for(const [cardId,m] of [...morphMap]){
    ...
    const docxFitKey=Math.round(w)+"x"+Math.round(h);
    if(el.dataset.docxFitKey!==docxFitKey){el.dataset.docxFitKey=docxFitKey;fitDocxPreview(el);}
    /* 形变覆盖层内的 iframe（网页链接）—— 与 syncPvContentZoom 同一方案：
       NoTab 100% 撑满 + CSS zoom 画布缩放同步（不裁剪、不破坏滚动） */
...
function drawMindConnections(scope){
  ...
  for(const ch of nodes){
    ...
    if(grow<1){
      ...
      ctx.save();
      /* 层级线退到结构层：同一分支淡色单线，不再用彩虹渐变抢语义关系线的注意力。 */
      ctx.strokeStyle=p.color;
      ctx.globalAlpha=isSel?.76:(SC.glass?.48:.34);
      ctx.lineWidth=(isSel?2.0:1.35)/z;ctx.lineCap="round";
      if(SC.glass){ctx.shadowColor=ch.color;ctx.shadowBlur=5/z;}
      /* 通用路线：主轴 S 形 / 跨轴圆角折线 */
      routeConnection(ctx,pa,cb,outSide,inpSide,p.color,(isSel?2.0:1.35)/z);
      ctx.stroke();
...
function exportPNG(){
  ...
  try{
    /* 1) 背景：画布当前 bgColor（含模板背景） */
    const dark=state.dark;
    const _BCM={default:dark?"#1a1a2e":"#ffffff",eye:dark?"#1a2a1e":"#c8e6c9",cream:dark?"#2a2a20":"#fff8e1",blue:dark?"#1a2030":"#e3f2fd",kraft:dark?"#2a2218":"#f4ecd8"};
    octx.fillStyle=_BCM[state.bgColorName]||(dark?"#1a1a2e":"#ffffff");
  ...
  try{
    ...
    const cur=curCanvas()||{name:"画布"};
    const name=(cur.name||"织见画布").replace(/[\\/:*?\"<>|]/g,"_");
    a.download=name+"-导出.png";
    a.href=url;
    document.body.appendChild(a);a.click();a.remove();
    toast("已导出 PNG（"+Wout+"×"+Hout+"）");
  }catch(e){
    console.warn("导出下载失败:",e);
    toast("导出失败（浏览器限制）");
  }
}
...
function configureWebPreviewFrame(iframe,url,title){
  /* Electron 的 webview 是独立访客页面，不受 iframe 的 X-Frame-Options / CSP
     frame-ancestors 限制；桌面版遇到知乎等站点时可避免“空白但无报错”。 */
  if(window.electronAPI&&window.electronAPI.isDesktop){
    const view=document.createElement("webview");
    view.className="pv-html";
    /* webview 不参与文档流高度计算：作为阅读舞台的绝对填充层，避免下半区留下空白。 */
    view.style.cssText="display:block;position:absolute;inset:0;width:100%;height:100%;min-height:0;border:none;background:#fff";
    view.src=url;
    view.setAttribute("partition","persist:weavision-web-preview");
    view.setAttribute("allowpopups","");
    /* 远程页始终禁用 Node；不再额外启用 renderer sandbox。
       少数站点会在该组合下中断主文档导航，造成“链接加载失败”的假报错。 */
    view.setAttribute("webpreferences","contextIsolation=yes,nodeIntegration=no");
    view.title=title||"网页材料预览";
    view.addEventListener("dom-ready",()=>applyEmbeddedPageZoom(view,view.__pvDesiredZoom||1));
    view.addEventListener("did-fail-load",e=>{
      /* webview 会为图片、统计脚本等子资源也发 did-fail-load。
         只有主文档导航失败才是网页预览失败，不能让一个子资源把整页替换成错误提示。 */
      if(e.isMainFrame&&e.errorCode!==-3)view.dispatchEvent(new Event("error"));
    });
    return view;
  }
  /* A1 浏览器成功基线：不混入桌面端访客页的额外权限与严格 Referer 策略。
     这里保持普通 iframe 的独立沙箱，交给站点自身完成其正常的资源加载。 */
...
function rejectsEmbeddedPreview(url){
  ...
  return host==="thepaper.cn"||host.endsWith(".thepaper.cn");
}
...
function drawNote(it,cc){
  ...
  const nr=SC.paper?3:SC.glass?14:SC.minimal?4:SC.neumorph?14:9;
  if(SC.neumorph){drawNeumorph(c,x,y,w,h,nr,sel,state.dark,state.bgColor);}else{
  ...
  c.save();
  roundRectPath(c,x,y,w,h,nr);
  c.strokeStyle=SC.paper?"rgba(104,76,42,.48)":SC.glass?"rgba(255,255,255,.72)":state.dark?"rgba(255,255,255,.16)":"rgba(0,0,0,.13)";
  ...
  if(SC.glass){
    ...
    const shine=c.createLinearGradient(x,y,x+w*.7,y+h);shine.addColorStop(0,"rgba(255,255,255,.34)");shine.addColorStop(.45,"rgba(255,255,255,.05)");shine.addColorStop(1,"rgba(255,255,255,0)");
...
function drawMindNode(it,cc){
  ...
  if(SC.neumorph){drawNeumorph(c,b.x,b.y,b.w,b.h,r,sel,state.dark,state.bgColor);}else{
  c.save();
  ...
  if(SC.paper){
    ...
    c.globalAlpha=1;c.fillStyle="rgba(255,255,255,.46)";c.fillRect(b.x+2/z,b.y+2/z,b.w-4/z,1.3/z);
    c.restore();
  }
  ...
  if(SC.paper){
    ...
    else{g.addColorStop(0,hexToRgba(it.color,depth===1?.24:.15));g.addColorStop(1,dc("rgba(255,255,255,.72)","rgba(30,34,48,.72)"));}
    c.fillStyle=g;c.fill();
    c.lineWidth=(depth===0?1.4:1)/z;c.strokeStyle=depth===0?"rgba(255,255,255,.72)":hexToRgba(it.color,.45);c.stroke();
  }else if(depth===0){
    if(SC.minimal){
  ...
  if(SC.glass){
    ...
    shine.addColorStop(0,"rgba(255,255,255,.46)");shine.addColorStop(.32,"rgba(255,255,255,.12)");shine.addColorStop(1,"rgba(255,255,255,0)");
    c.fillStyle=shine;c.fillRect(b.x,b.y,b.w,b.h);
    c.fillStyle="rgba(255,255,255,.11)";c.fillRect(b.x+1/z,b.y+1/z,b.w-2/z,1.1/z);c.restore();
  }else if(SC.paper){
    c.save();roundRectPath(c,b.x+1/z,b.y+1/z,b.w-2/z,b.h-2/z,r);c.clip();
    ...
    c.globalAlpha=.36;c.fillStyle="rgba(255,255,255,.62)";c.fillRect(b.x+4/z,b.y+3/z,b.w-8/z,1/z);
    if(depth<=1){
      ...
      c.globalAlpha=.28;c.fillStyle="rgba(255,255,255,.86)";c.beginPath();c.moveTo(b.x+b.w-fold,b.y+b.h);c.lineTo(b.x+b.w-fold,b.y+b.h-fold);c.lineTo(b.x+b.w,b.y+b.h-fold);c.closePath();c.fill();
    }
    c.restore();
  }
  ...
  const padL=14;
  if(SC.neumorph) c.fillStyle=depth===0?dc("#3a3a3e","#e8e8ec"):dc("#555560","#b0b0b8");
...
function drawAnnotation(it,b){
  ...
  if(it.detail){
    ...
    ctx.save();
    /* 展开态仅加细描边区分；常态无填充、无高光、无阴影 */
    if(expanded){
      ctx.lineWidth=1.2/z;ctx.strokeStyle="rgba(45,95,211,.45)";
      roundRectPath(ctx,box.x,box.y,box.w,box.h,box.h/2);ctx.stroke();
    }
    const cx=box.x+box.w/2,cy=box.y+box.h/2;
    ctx.strokeStyle=state.dark?"#9aa0ac":"#8a90a0";
    ctx.lineWidth=1.35/z;ctx.lineCap="round";ctx.lineJoin="round";
    if(expanded){
  ...
  /* 有跃迁：提供可点击的明确入口，而不是只展示目的地提示。 */
  if(it.jumpTo){
    const jb=jumpBounds(it,b);
    ctx.save();ctx.fillStyle=dc("rgba(255,255,255,.78)","rgba(47,56,75,.86)");
    roundRectPath(ctx,jb.x,jb.y,jb.w,jb.h,4/z);ctx.fill();
    ctx.strokeStyle=dc("rgba(42,94,195,.4)","rgba(147,180,245,.42)");ctx.lineWidth=1/z;ctx.stroke();
    ctx.fillStyle=dc("#285bbc","#c7dcff");ctx.font="600 9px "+FONT;ctx.textAlign="center";ctx.textBaseline="middle";
    ctx.fillText("↗ 跳转",jb.x+jb.w/2,jb.y+jb.h/2+.4/z);ctx.restore();
  }
...
function jumpBounds(it,b){
  const z=state.camera.zoom,w=40/z,h=16/z;
  return{x:b.x+7/z,y:b.y+b.h-h-5/z,w,h};
}
/* ============================================================
...
function drawFileCard(it,cc){
  ...
  if(SC.neumorph){drawNeumorph(c,b.x,b.y,b.w,b.h,SC.radius,sel,state.dark,state.bgColor);}else{
  c.save();
  ...
  c.lineWidth=(SC.paper?1.1:1)/z;c.strokeStyle=SC.paper?"rgba(104,76,42,.52)":SC.glass?"rgba(255,255,255,.76)":SC.borderColor;
  ...
  if(it.previewOpen){
    ...
    if(cb){
      ...
      const baseCol=dc("rgba(45,95,211,.16)","rgba(255,255,255,.2)");
...
function enterFocus(id){
  ...
  const b=itemBounds(it);
  const fullW=detailLayout&&detailLayout.id===it.id?detailLayout.width:b.w;
  const fullH=detailLayout&&detailLayout.id===it.id?detailLayout.baseH+detailLayout.contentH:b.h;
...
let detailEditSession=null; /* 编辑开始时的内容与展开状态，用于避免“未改却变宽” */
...
function finishMdEdit(){
  ...
  const next=detailTextarea.value;
  const changed=!!(it&&it.detail!==next);
  const wasExpanded=!!(detailEditSession&&detailEditSession.id===detailEditId&&detailEditSession.wasExpanded);
  if(changed)pushHistory("编辑展开内容");
  if(it)it.detail=next;
  closeMdModal();
  detailEditId=null;
  activeMdEditor=null;
  /* 未作编辑时绝不触碰画布几何；编辑后从原始卡片尺寸重新计算阅读层。 */
  if(changed&&it){
    if(detailLayout&&detailLayout.id===it.id){
    ...
    if(next.trim())expandDetailInPlace(it);
  }else if(wasExpanded&&it&&detailItemId!==it.id){
    expandDetailInPlace(it);
  }
  detailEditSession=null;
  render();saveState();
}
...
detailTextarea.addEventListener("input",()=>{
  /* 编辑中的草稿只驻留在编辑器；提交时一次性写回，
     才能准确判断是否修改并重新计算画布阅读层尺寸。 */
...
detailTextarea.addEventListener("keydown",e=>{
  ...
  if(e.key==="Escape"){
    e.preventDefault();
    closeMdModal();detailEditId=null;activeMdEditor=null;detailEditSession=null;
    renderDock(true);render();
  }
  /* Ctrl+B / Ctrl+I 快捷键 */
