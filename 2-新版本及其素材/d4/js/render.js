...
function render(){
  ...
  if(typeof syncDetailReadDom==="function")syncDetailReadDom(); /* D4: 展开内容阅读层同步 */
...
function drawMindConnections(scope){
  ...
  for(const ch of nodes){
    ...
    if(grow<1){
      ...
      ctx.save();
      /* D4: mind link level — width primary, no halo */
      ...
      ctx.stroke();
      /* D4: highlight level — inner light line */
...
function drawMindNode(it,cc){
  ...
  /* D4: detail toggle is on LEFT side now — doesn't affect text width */
...
function drawAnnotation(it,b){
  ...
  if(it.detail){
    /* D4: left-side tab button — distinct color, prominent, sticks out from left edge */
...
/* D4: removed detail modal handlers */

/* D4: Detail state variables (restored after truncation) */
