...
============================================================ */
const ZHIJIAN_AI_API_VERSION="1.2";   /* 1.2: 计划校验、稳定排版、原子 batch 与显式替换确认 */
...
function aiBuildCanvas(plan){
  ...
  for(const spec of notes){
    ...
    note.annotation=spec.annotation||""; /* D4: note.detail removed — mind-node only */
  ...
  for(const spec of attachments){
    ...
    card.annotation=spec.annotation||""; /* D4: card.detail removed — mind-node only */
...
function aiExecute(command){
  try{
    ...
    switch(op){
      ...
      case "create_note":aiActivate(command.projectId||state.activeProjectId,command.canvasId||state.activeCanvasId);pushHistory();value=addNote(command.x||0,command.y||0,command.markdown||command.text||"",command.color||state.noteColor); /* D4: note.detail removed */aiCommit();break;
