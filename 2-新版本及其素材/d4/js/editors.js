...
/* D4: Detail inline editor — reuses noteEditor + noteFormatBar */
...
function dockContent(st){
  ...
  if(st==="select"&&sel){
    ...
    if(sel.type==="mindLink"){
      /* D4: mind links also support level/shape — stored on child node */
      ...
      out.push(dkSep());
      out.push(dkBtn({icon:ICON.edit,label:"编辑",title:"编辑内容 (双击)",fn:()=>{if(sel.type==="note")openTextEditor(sel);else{state.selected=sel.id;/* D4: openDetail removed */}}}));
    }else if(sel.type==="link"){
    /* D4: link dock — level, shape, relation, annotate, focus */
    var lvlLabels={normal:"普通",emphasis:"强调",highlight:"醒目"};
...
/* D4: dp-t/dp-sel/detailTextarea handlers all removed */
/* 批注：A键，选中元素后添加常驻小字 */
...
function syncDetailReadDom(){
  ...
  if(!it.detail){
    /* D4: Show "double-click to edit" placeholder when no detail content */
...
function hitNodeControl(wx,wy){
  ...
  return null;
}/* D4: detail keydown listener removed *//* 连接待选态：点击"连接"按钮时若仅选中 1 个元素，
...
function drawLinks(scope){
  ...
  for(const l of state.links){
    ...
    const LK=styleCfg();
    /* D4: line level — width is the primary distinguisher, NOT glow/halo */
    ...
    /* D4: higher levels use more vivid color */
    ...
    /* D4: highlight level — inner light line (thin lighter stroke on top of thick line) */
...
function hitRelationLink(wx,wy){
  ...
  for(let i=state.links.length-1;i>=0;i--){
    ...
    if((a&&a.type==="mindNode"&&!isMindNodeVisible(a))||(b&&b.type==="mindNode"&&!isMindNodeVisible(b)))continue;
    const curve=linkCurve(l);if(!curve)continue;
    /* D4: use correct control points based on direction (horizontal vs vertical S-curve) */
    var p1x,p1y,p2x,p2y;
    if(curve.horizontal){
