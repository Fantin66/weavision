...
function saveState(){
  try{
    const data={
      ...
      ui:{sideCollapsed:state.sideCollapsed,dark:state.dark,bgPattern:state.bgPattern,bgColorName:state.bgColorName,mindMode:state.mindMode,mindColorMode:state.mindColorMode,layoutType:state.layoutType,fontPreset:state.fontPreset,stylePreset:state.stylePreset,reducedMotion:state.reducedMotion},
      savedAt:Date.now(),
    };
...
function applyTheme(){
  ...
  /* E5: refresh logo on theme change */
...
const LOGO_PRESETS={
  1:{label:"穿线元素",light:"g1/assets/logos/logo1-light.png",dark:"g1/assets/logos/logo1-dark.png",
     cnLight:"g1/assets/logos/logo01-中文字标-light.png",cnDark:"g1/assets/logos/logo01-中文字标-dark.png",
     enLight:"g1/assets/logos/logo01-英文字标-light.png",enDark:"g1/assets/logos/logo01-英文字标-dark.png",
     comboLight:"g1/assets/logos/logo01-横版组合-light.png",comboDark:"g1/assets/logos/logo01-横版组合-dark.png"},
  2:{label:"聚焦轨道",light:"g1/assets/logos/logo2-light.png",dark:"g1/assets/logos/logo2-dark.png",
     cnLight:"g1/assets/logos/logo02-中文字标-light.png",cnDark:"g1/assets/logos/logo02-中文字标-dark.png",
     enLight:"g1/assets/logos/logo02-英文字标-light.png",enDark:"g1/assets/logos/logo02-英文字标-dark.png",
     comboLight:"g1/assets/logos/logo02-横版组合-light.png",comboDark:"g1/assets/logos/logo02-横版组合-dark.png"},
  3:{label:"叠合元素",light:"g1/assets/logos/logo3-light.png",dark:"g1/assets/logos/logo3-dark.png",
     cnLight:"g1/assets/logos/logo03-中文字标-light.png",cnDark:"g1/assets/logos/logo03-中文字标-dark.png",
     enLight:"g1/assets/logos/logo03-英文字标-light.png",enDark:"g1/assets/logos/logo03-英文字标-dark.png",
     comboLight:"g1/assets/logos/logo03-横版组合-light.png",comboDark:"g1/assets/logos/logo03-横版组合-dark.png"},
};
let logoPreset=3;
...
function init(){
  ...
  openDB().then(()=>{
    ...
    renderSidePanel();
    render();
    /* G1: 启动动画收尾 — 最低展示 1.5s 后淡出 */
    var _splashT0=Date.now();
    function hideSplash(){
      var el=document.getElementById("splashScreen");
      if(!el)return;
      var elapsed=Date.now()-_splashT0;
      if(elapsed<1500){setTimeout(hideSplash,1500-elapsed);return;}
...
(function(){
  ...
  if(img)img.src="g1/assets/logos/splash-"+sl+".png";  /* G1: 用透明母版 icon */
})();
...
function renderSettingsContent(catId,content){
  if(catId==="general"){
    content.innerHTML=
      ...
      '<div style="margin-bottom:20px"><div style="font-size:12px;color:var(--ink-dim);margin-bottom:6px">全屏模式</div>'+
      '<button id="setFullscreen" style="padding:8px 16px;border:1px solid var(--card-border);border-radius:8px;background:var(--surface);color:var(--ink);cursor:pointer;font:600 12px var(--font)">切换全屏 (F11)</button>'+
      '<div style="font-size:11px;color:var(--ink-faint);margin-top:4px">桌面应用安装后默认不全屏，可在此切换</div></div>'+
      ...
      '<div style="margin-bottom:20px"><div style="font-size:12px;color:var(--ink-dim);margin-bottom:6px">自动保存</div>'+
      '<div style="font-size:12px;color:var(--ink)">每 30 秒自动保存 · 关闭页面时自动保存</div>'+
      '<div style="font-size:11px;color:var(--ink-faint);margin-top:4px">未来可选：关闭 / 10秒 / 30秒 / 60秒</div></div>'+
    ...
    var fs=content.querySelector("#setFullscreen");if(fs)fs.onclick=function(){toggleImmersive();};
    ...
    content.innerHTML=
      ...
      '<div style="font-size:12px;color:var(--ink)">当前：浏览器 localStorage（网页版）</div>'+
      '<div style="font-size:11px;color:var(--ink-faint);margin-top:4px">桌面版将支持自定义文件存储位置</div></div>'+
      '<div style="margin-bottom:20px"><div style="font-size:12px;color:var(--ink-dim);margin-bottom:6px">文件存储位置（桌面版预留）</div>'+
      '<div style="display:flex;gap:8px"><input style="flex:1;height:34px;padding:0 10px;border:1px solid var(--card-border);border-radius:8px;background:var(--surface);color:var(--ink);font:12px var(--font)" value="~/Documents/织见/" readonly>'+
      '<button style="padding:0 12px;border:1px solid var(--card-border);border-radius:8px;background:var(--surface);color:var(--ink-dim);cursor:pointer;font:12px var(--font)">选择…</button></div>'+
      '<div style="font-size:11px;color:var(--ink-faint);margin-top:4px">附件、导出文件、备份将存到此目录</div></div>'+
    ...
    if(eb)eb.onclick=function(){toast("AI Skill 导出功能开发中");};
  }
  else if(catId==="about"){
    content.innerHTML=
      ...
      '<div style="font-size:12px;color:var(--ink-dim);margin-top:4px">思维关系板 · Demo E10</div>'+
      '<div style="font-size:11px;color:var(--ink-faint);margin-top:8px;font-style:italic">织连万象，见聚一隅</div>'+
      '</div>'+
      '<div style="font-size:11px;color:var(--ink-dim);text-align:center;margin-top:16px">本地优先 · 数据存储于浏览器 localStorage<br>桌面版即将推出</div>';
  }
}
