...
function render(){
  ...
  if(typeof syncDetailReadDom==="function")syncDetailReadDom(); /* D3: 展开内容阅读层同步 */
...
function drawAnnotation(it,b){
  ...
  if(it.detail){
    /* D3: left-side tab button — distinct color, prominent, sticks out from left edge */
...
function exitFocus(){
  ...
  if(detailItemId!==null&&state.focusMode&&detailItemId===state.focusMode.id){
    if(typeof collapseDetailInPlace==="function")collapseDetailInPlace();
  }
...
/* D3: removed detail modal handlers */

/* D3: Detail state variables (restored after truncation) */
let detailItemId=null;
let detailEditId=null;
let detailEditSession=null;
let detailLayout=null,detailMotionFrame=0;
const detailPanel=null;
const detailTextarea=null;
