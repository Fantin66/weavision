...
============================================================ */
function render(){
  /* F10: 每帧同步 canvas 尺寸到 #board 实际大小 */
  const bw=board.clientWidth||1, bh=board.clientHeight||1;
  if(bw!==W||bh!==H){
    W=bw;H=bh;
  ...
  /* F15 诊断：显示 #board 和 canvas 尺寸，定位绿带/偏移根因 */
  {
    const dbg=document.getElementById("dbg-hud");
    if(dbg){
      const r=board.getBoundingClientRect();
      const cam=state.camera;
      const pv=document.getElementById("previewLayer");
      dbg.textContent=`board:${board.clientWidth}(L${Math.round(r.left)}R${Math.round(r.right)})vw=${window.innerWidth}|W=${W}|cam:x${Math.round(cam.x)}z${cam.zoom.toFixed(2)}|side=${state.sideCollapsed}|cls=${board.classList.contains("side-collapsed")}|style=${state.stylePreset}|pvChildren=${pv?pv.children.length:"-"}|pvDisp=${pv?pv.style.display:"-"}|bg=${state.bgColor}`;
    }
  }
...
function syncMorphDom(){
  ...
  if(layer){
    for(const child of [...layer.children]){
      /* F18: 移除 !child.dataset.card 的 continue 跳过 — 这导致无 card 的残留 .pv-morph 永远不删，
         previewLayer 保持 display:block（13 个残留子元素）在 canvas 上方形成毛玻璃遮罩 = 绿带 bug */
      if(child.classList&&child.classList.contains("pv-morph")){
        const cid=child.dataset.card;
        if(!cid||!morphMap.has(cid)){child.remove();}
      }
    }
    /* F12: previewLayer 无子元素时隐藏，有子元素时才显示。
       原代码 previewLayer 始终可见（z-index:5 在 canvas 上方），即使为空也可能干扰渲染。 */
...
function routeConnection(c,a,b,outSide,inpSide,color,width,shapeOverride){
  const z=state.camera.zoom;
  const lt=shapeOverride||layoutLinetype();
...
function detailCollapseBounds(it,b){
  /* E5: same position when expanded */
  const z=state.camera.zoom;
  const coreH=detailBaseHeight(it,b),w=22/z,h=22/z;
  return{x:b.x-w-3/z,y:b.y+(coreH-h)/2,w,h};
}
