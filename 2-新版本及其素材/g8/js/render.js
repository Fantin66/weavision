    ...
    if(mBody){
      ...
      if(iframe&&stage){
        ...
        if(mb){
          ...
          iframe.style.width="375px";
          iframe.style.height="100%";
          /* G7: Electron webview 不响应 CSS zoom，必须用 applyEmbeddedPageZoom */
          applyEmbeddedPageZoom(iframe,ratio);
          stage.style.overflowX="auto";stage.style.overflowY="hidden";
          stage.style.justifyContent="center";
        }else{
          /* G7: webview 是替换元素，需要显式像素尺寸。
             优先用 stage 的 clientHeight，fallback 用 morph 高度减去工具栏 */
          var sw=Math.max(1,stage.clientWidth);
          var sh=stage.clientHeight;
          if(sh<=0){sh=Math.max(1,(parseInt(el.style.height)||0)-32);}
