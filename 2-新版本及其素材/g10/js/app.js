...
function renderSettingsContent(catId,content){
  if(catId==="general"){
    ...
    if(sl)sl.onclick=function(){_themeToken++;state.dark=false;_applyThemeInner();render();saveState();showSettings();};
    var sd=content.querySelector("#setDark");
    if(sd)sd.onclick=function(){_themeToken++;state.dark=true;_applyThemeInner();render();saveState();showSettings();};
