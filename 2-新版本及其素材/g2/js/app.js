...
function renderSettingsContent(catId,content){
  if(catId==="general"){
    content.innerHTML=
      ...
      (!state.autoTheme?'<button id="setLight" style="padding:8px 14px;border:1px solid '+(!state.dark?"var(--accent)":"var(--card-border)")+';border-radius:8px;background:'+(!state.dark?"var(--accent-soft)":"var(--surface)")+';color:'+(!state.dark?"var(--accent)":"var(--ink)")+';cursor:pointer;font:600 12px var(--font)">☀ 亮色</button>'+
      ...
      '</div><div style="font-size:11px;color:var(--ink-faint);margin-top:4px">开启后自动跟随操作系统的亮色/暗色模式</div></div>'+
      ...
      '<div style="font-size:12px;color:var(--ink)">每 30 秒自动保存 · 关闭页面时自动保存</div></div>'+
    ...
    if(sl)sl.onclick=function(){state.autoTheme=false;state.dark=false;applyTheme();render();saveState();showSettings();};
    var sd=content.querySelector("#setDark");
    if(sd)sd.onclick=function(){state.autoTheme=false;state.dark=true;applyTheme();render();saveState();showSettings();};
