...
function aiApplyStyle(value,explicitFont){
  ...
  else if(key==="paper")state.fontPreset="handwritten";
...
function aiRestoreState(serialized){
...
function aiBuildCanvas(plan){
  ...
  for(const spec of attachments){
    ...
    if(spec.previewOpen===true)card.previewOpen=true;
...
function aiExecute(command){
  try{
    ...
    switch(op){
      ...
      case "create_note":aiActivate(command.projectId||state.activeProjectId,command.canvasId||state.activeCanvasId);pushHistory();value=addNote(command.x||0,command.y||0,command.markdown||command.text||"",command.color||state.noteColor); /* E5: note.detail removed */aiCommit();break;
      ...
      case "toggle_detail":{const it=aiItem(command.itemId||command.id);if(it.type!=="mindNode")aiFail("展开控制仅适用于导图节点");pushHistory();toggleDetailInPlace(it);value={itemId:it.id,expanded:detailItemId===it.id};break;}
      case "set_detail":{const it=aiItem(command.itemId||command.id);if(it.type!=="mindNode")aiFail("展开控制仅适用于导图节点");const want=command.expand!==undefined?!!command.expand:true;pushHistory();if(want&&detailItemId!==it.id){if(detailItemId!==null)collapseDetailInPlace();expandDetailInPlace(it,true);}else if(!want&&detailItemId===it.id){collapseDetailInPlace();}render();value={itemId:it.id,expanded:detailItemId===it.id};break;}
      ...
      case "set_layout":{state.layoutType=normalizeAiLayout(command.layout);autoLayout();aiCommit();value={layout:state.layoutType};break;}
