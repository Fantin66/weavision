...
function syncMorphDom(){
  ...
  /* G5: 自动展开 — previewOpen=true 但不在 morphMap 里的 fileCard，自动创建 morph 入口 */
  if(layer){
    for(const it of (state.items||[])){
      if(it.type==="fileCard"&&it.previewOpen&&it.fileId){
        ...
        if(!morphMap.has(cid)){
          ...
          el2.className="pv-morph";el2.dataset.card=cid;
          const tools2=document.createElement("div");tools2.className="pv-morph-tools";
          const body2=document.createElement("div");body2.className="pv-morph-content";
          el2.appendChild(tools2);el2.appendChild(body2);
          layer.appendChild(el2);
          morphMap.set(cid,{el:el2,body:body2,fileId:it.fileId,token:++morphToken});
        }
      }
    }
...
function exportPNG(){
  ...
  try{
    ...
    octx.restore();
    /* G2: 水印 — 用文字（不用图片，避免 file:// 污染 canvas 导致 toDataURL 失败） */
    try{
      var wmSl=parseInt(localStorage.getItem("zhijian-logo"))||3;
      octx.save();
