...
function ensureTutorV6(){
  const existing=state.projects.find(p=>p.name==="知见学堂");
...
function ensureTutorProject(){
  ...
  state.projects=state.projects.filter(p=>p.name!=="织见学堂"&&p.name!=="知见学堂"&&p.name!=="回归测试");
  const removedOld=countBefore!==state.projects.length;
  const existing=state.projects.find(p=>p.name==="知见学堂");
  if(existing&&existing.tutorialVersion===TUTORIAL_VERSION&&(existing.canvases||[]).length===7){
  ...
  const makeFileCard=(canvas,file,x,y,w=200,h=58,annotation="",previewOpen=false)=>{
    const item={id:uid++,type:"fileCard",x,y,w,h,fileId:file.id,kind:file.kind,thumb:null,tw:1,th:1,annotation,previewOpen,birth:performance.now()};
    canvas.items.push(item);return item;
  };
  ...
  const c4Theme=makeNode(c4,"昼夜主题：自动跟随系统",-460,-120,"#2d5fd3",c4Root,
    ...
    "默认 autoTheme = true。");
  const c4BgColor=makeNode(c4,"背景色系：neutral / warm / cool / green",-460,40,"#319b77",c4Root,
    "## 背景色系\n\n四种色系驱动底色和漂移光晕的调色板：\n\n- **neutral**：中性灰蓝\n- **warm**：暖米色\n- **cool**：冷青蓝\n- **green**：柔绿\n\n每种色系都有昼/夜两套配色。",
    "色系决定漂移色的基调。");
  const c4Texture=makeNode(c4,"背景纹理：grid / dots / lines / blank",230,-120,"#319b77",c4Root,
  ...
  const c4Font=makeNode(c4,"字体预设",230,180,"#7890e5",c4Style,
    "## 字体\n\n12 种 Google Fonts 字体可选，涵盖中英文。\n\n部分样式会自动切换字体：minimal → serif，paper → handwritten（已移除）。",
    "在设置面板切换字体。");
  ...
  const c7Root=makeNode(c7,"织见：现状与规划",-10,-280,"#2d5fd3",null,
    "# 现状与规划\n\n织见从 2024 年概念诞生到现在，经历了 C1–G5 多个版本迭代。\n\n这里记录已完成的工作、正在优化的问题、以及未来规划。",
    "持续迭代中。");
  const c7Done=makeNode(c7,"已完成：C1–G5",-460,-100,"#319b77",c7Root,
    "## 已完成里程碑\n\n- **C1–C8**：基础画布、节点、连接、便签、附件\n- **C9–C12**：形变展开、语义关系、布局系统\n- **C13**：精简重构，移除展开内容（将回归）\n- **E5**：Win11 Fluent Design 材质系统\n- **F1–F19**：七种样式预设、背景纹理、漂移色、Mica\n- **G1–G5**：桌面应用（Electron）、导入导出（.fantin）、自动主题、AI 接口 v1.3",
    "核心功能已完成。");
  const c7Optimize=makeNode(c7,"待优化",230,-100,"#d78f36",c7Root,
    "## 待优化\n\n- 玻璃样式的绿带问题（低优先级）\n- SVG 矢量导出\n- 大画布性能（animateCamera 节流、annotationPulse）\n- 顶部菜单补全",
    "持续打磨中。");
  const c7Develop=makeNode(c7,"待开发",-460,80,"#6b73cc",c7Root,
    "## 待开发\n\n- **形变机制回归**：仅限 mind-node，精简实现\n- **AI 自主画板构建**：利用 typed relations + morph 自动画板\n- **OPML 导出**：标准格式互通\n- **Web 预览安全**：XFO/CSP 防护\n- **文件关联**：.fantin 双击打开",
    "下一阶段重点。");
  ...
  /* ── 便签 ── */
  makeNote(c7,"## 版本命名规则\n\n- **C** 系列：Canvas（画布功能）\n- **E** 系列：Experience（材质体验）\n- **F** 系列：Fluent（样式预设）\n- **G** 系列：Desktop（桌面应用）\n\n> 每个大版本 = 一个完整的能力域。",-460,220,"#dbeafe",360,130,"版本命名逻辑。");
  /* ── 关系 ── */
  ...
  /* detailItemId：展开活动画布（c1）的核心节点，展示就地展开效果 */
  if(state.activeCanvasId===c1.id){
    /* detailItemId 是全局变量（render.js），运行时控制哪个 mindNode 的详情展开 */
    detailItemId=c1Root.id;
  }
