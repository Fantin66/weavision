...
function render(){
  ...
  /* F15 诊断：显示 #board 和 canvas 尺寸，定位绿带/偏移根因 */
  {
    ...
    if(dbg){
      ...
      dbg.textContent=`board: ${board.clientWidth}×${board.clientHeight} (L${Math.round(r.left)} R${Math.round(r.right)}) | canvas: ${canvas.style.width||"(CSS 100%)"} × ${canvas.style.height||"(CSS 100%)"} | W=${W} H=${H} | sideCollapsed=${state.sideCollapsed} | style=${state.stylePreset} | dataStyle=${document.documentElement.dataset.style} | hasCollapsedClass=${board.classList.contains("side-collapsed")} | bgColor=${state.bgColor}`;
    }
  }
