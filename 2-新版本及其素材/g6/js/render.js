...
/* G6: 跃迁按钮闪烁 — 仅当画布上有 jumpTo 元素时以低频重绘 */
let _jumpPulseTimer=null;
function _jumpPulseCheck(){
  var has=false;
  for(var i=0;i<state.items.length;i++){
    if(state.items[i].jumpTo){has=true;break;}
  }
  if(!has){clearInterval(_jumpPulseTimer);_jumpPulseTimer=null;return;}
  requestRender();
}
function ensureJumpPulse(){
  if(_jumpPulseTimer)return;
  _jumpPulseTimer=setInterval(_jumpPulseCheck,400);
}
...
function showJumpConfirm(item){
  ...
  el.innerHTML='<span style="margin-right:4px">↗</span>跃迁到「<strong>'+cn+'</strong>」<span class="jc-hint">点击确认</span>';
  var dk=state.dark;
...
let detailLayoutMap=new Map(),detailMotionFrame=0;
