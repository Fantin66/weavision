...
function init(){
  ...
  applyFontPreset();
  resize();
  /* F17: 移除 ResizeObserver — 它可能造成反馈循环
    （ResizeObserver → resize → applyResponsive → applySide → #board 尺寸变 → ResizeObserver 再触发）。
     F10 的每帧 board.clientWidth 检查已足够同步 canvas 尺寸。 */
  openDB().then(()=>{
    const resetForTest=SIMPLE_TEST_MODE&&resetLocalArchiveForSimpleTest();
    const has=resetForTest?false:loadState();
    /* F15: loadState 加载了 sideCollapsed/stylePreset 但没调 applySide，
       导致 .side-collapsed class 没加到 #board → left 没变 0 → 右侧空一块 */
    applySide();
    applyTheme();
    /* F19: 强制清理残留 morph 状态 — loadState 加载了 previewOpen=true 的卡片，
       但 morphMap 是空的（运行时结构不持久化）。残留的 previewOpen 导致
       syncMorphDom 主循环把 DOM 元素加回 previewLayer → 13 个残留子元素 → 绿带。 */
    if(has){
      for(const p of state.projects||[]){
        for(const c of p.canvases||[]){
          for(const it of c.items||[]){
            if(it.type==="fileCard"&&it.previewOpen){it.previewOpen=false;}
          }
        }
      }
    ...
    if(!has){
      ...
      restoreFiles().then(()=>{
        renderSidePanel();
        /* F19: restoreFiles/syncPvDom 可能往 previewLayer 填充残留元素，
           在 render 前清空 morphMap + previewLayer，杜绝 13 个残留子元素 */
        if(typeof morphMap!=="undefined")morphMap.clear();
        const pv2=document.getElementById("previewLayer");
        if(pv2){pv2.innerHTML="";pv2.style.display="none";}
        render();fitAll();
        syncPvDom();
        /* syncPvDom 后再清一次（它可能重新填充） */
        if(typeof morphMap!=="undefined")morphMap.clear();
        const pv3=document.getElementById("previewLayer");
        if(pv3){pv3.innerHTML="";pv3.style.display="none";}
        render();
      });
    }
    renderSidePanel();
...
/* F17: 模块级 ResizeObserver 也移除 — 同反馈循环原因 */
