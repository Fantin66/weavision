...
function syncDetailReadDom(){
  ...
  detailReadLayer.style.display="none";  /* F5: 默认隐藏，有内容时再显示 */
  const it=detailItemId!==null&&state.items.find(x=>x.id===detailItemId);
  if(!it){detailReadLayer.innerHTML="";return;}
  if(!it.detail){
    /* E5: Show "double-click to edit" placeholder when no detail content */
    ...
    detailReadLayer.style.display="block";  /* F5 */
