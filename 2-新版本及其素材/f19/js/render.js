...
function syncMorphDom(){
  ...
  if(layer){
    /* F19: 直接扫描 DOM — 只保留 morphMap 里且 previewOpen=true 的元素，其余全删。
       不依赖 morphMap.size 或 hasOpen 条件（可能有残留条目导致条件不满足）。 */
    const legitEls=new Set();
    for(const [cardId,m] of morphMap){
      const card=state.items.find(i=>i.type==="fileCard"&&String(i.id)===cardId);
      if(card&&card.previewOpen){legitEls.add(m.el);}
    }
    for(const child of [...layer.children]){
      if(!legitEls.has(child)){child.remove();}
    }
