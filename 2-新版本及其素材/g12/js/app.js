...
function debounce(fn,wait){let t=null;const d=function(){const ctx=this,args=arguments;if(t)clearTimeout(t);t=setTimeout(()=>fn.apply(ctx,args),wait);};d.flush=function(){if(t){clearTimeout(t);t=null;fn.apply(ctx,[]);}};d.cancel=function(){if(t){clearTimeout(t);t=null;}};return d;}
...
document.addEventListener("keydown",e=>{
  ...
  if(e.altKey&&!e.ctrlKey&&!e.metaKey&&(e.key==="e"||e.key==="Æ")){
    ...
    render();saveStateDebounced();toast("deleted detail content");
    return;
  }
  /* F 键：聚焦模式 */
...
function init(){
  ...
  setupAutoSave();
  /* H1 任务5: 关闭/刷新前强制立即存档（绕过防抖），避免丢最后 1-2 秒改动 */
  window.addEventListener("beforeunload",saveState);
    ...
    if(sp)sp.onclick=function(){
      if(window.electronAPI&&window.electronAPI.selectDirectory){
        window.electronAPI.selectDirectory().then(function(p){
          if(p){state.storagePath=p;saveStateDebounced();showSettings();toast("存储位置已设置："+p);}
        });
      }
    };
  }
