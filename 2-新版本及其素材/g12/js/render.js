...
function syncPvContentZoom(el,pv,z){
  ...
  if(iframe&&stage){
    ...
    if(mb){
      ...
      /* H5: 优先显示完整——尺寸补偿(100/eff) + zoom=eff → 框撑满、整页等比缩完整可见 */
      var eff=userZoom*0.7;
    ...
    if(mBody){
      ...
      if(iframe&&stage){
        ...
        if(mb){
          ...
          /* H5: 优先"显示完整"——尺寸补偿 sw/eff + zoom=eff → 框始终撑满、整页内容等比缩完整可见。
             代价：跨域 iframe 内容不随 +/- 重算缩放（+/- 基本无可见反应），用户已接受。 */
          var sw=Math.max(1,stage.clientWidth);
          var sh=stage.clientHeight;
          if(sh<=0){sh=Math.max(1,(parseInt(el.style.height)||0)-32);}
          var eff=userZoom*0.7;
          iframe.style.width=(sw/eff)+"px";
          iframe.style.height=(sh/eff)+"px";
          applyEmbeddedPageZoom(iframe,eff);
          stage.style.overflow="auto";
