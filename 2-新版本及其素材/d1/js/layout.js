...
/* 空间方向决策：默认向右；空间不足时评估左/下/双向，返回 {dir,reason}
   判定依据 = 树的估算宽深 vs 画布物理可用区（视口只是参照） */
function decideLayoutDirection(spec){
  const t=state.layoutType||"logic";
  /* 重新排版必须可预期、可复现：严格遵守用户当前选定的布局，
     不再按临时缩放和视口把右向图改成双向或向下。 */
  if(["logic","org","fishbone","timeline"].includes(t))return{dir:t,reason:"使用当前布局"};
  /* 仅对逻辑图（右/左）做动态方向；其余布局类型由用户显式选择，不自动切换 */
  if(t!=="logic")return{dir:t,reason:"用户显式布局"};
  if(!state.items.length)return{dir:"logic",reason:"空画布"};
  const root=layoutRoot();if(!root)return{dir:"logic",reason:"无根"};
  /* 估算树的宽/深 */
  const totalH=subTreeH(root,spec.gapY);       /* 所有层排成一列的垂直总高 */
  const rootW=itemBounds(root).w;
  const estDepth=estimateDepth(root);
  /* 水平总宽 = 根宽 + 深度×(间距+节点宽)，加一层兄弟宽度裕量（保守估算，防拥挤） */
  const estW=rootW+estDepth*(spec.gapX(1)+rootW)+rootW*0.8;
  /* 视口世界尺寸（缩放后）——作为"当前可看范围"的参照 */
  const vw=W/state.camera.zoom, vh=H/state.camera.zoom;
  /* 垂直稠密：总高超过视口 → 需要疏散 */
  const dense=totalH>vh*0.95;
  /* 水平稠密：估算宽超过视口右半 → 需要换向/双向 */
  const wide=estW>vw*0.72;
  if(!dense&&!wide)return{dir:"logic",reason:"逻辑图"};
  if(dense||wide){
    /* 双向分叉治两种拥挤：垂直 → 一分为二；水平 → 左右分摊 */
    const halfH=totalH/2;
    const halfW=estW/2;
    if(halfH<=vh*0.9&&halfW<=vw*0.85)return{dir:"both",reason:(dense?"垂直稠密":"水平稠密")+"→双向分叉均衡"};
    if(!wide)return{dir:"down",reason:"垂直超载→下方展开"};   /* 仅垂直稠密用下方 */
  }
  /* 右侧不足且不垂直稠密：向左镜像 */
  return{dir:"logic",reason:"逻辑图保持用户拖拽方向"};
}
/* 估算树的最大深度（层数），用于方向决策的宽度估算 */
function estimateDepth(n){
  if(!n)return 1;
  let max=1;
  for(const k of kidsOf(n)){max=Math.max(max,1+estimateDepth(k));}
  return max;
}
...
function renderSelBar(sel){
  ...
  if(sel.type==="fileCard"){
    ...
    if(f) selbar.appendChild(sbtn(ICON.fullscreen,"全屏预览 (Alt+F)",()=>openFullscreen(f)));
  }
  if(sel.type==="note"){
...
function closeEditor(cancel){
  if(editingDetailId!==null){
    ...
    if(it&&!cancel){const v=noteEd.value;if(v!==it.detail){pushHistory("edit detail");it.detail=v;}}
    editingDetailId=null;noteEd.style.display="none";noteFormatBar.style.display="none";
...
function showCtxMenu(e,item){
  ...
  if(item){
    ...
    {const e2=mkItem(ICON.plus,item.detail?"编辑展开":"添加展开","E",false);e2.onclick=()=>{hideCtxMenu();state.selected=item.id;toggleDetailInPlace(item);};body.appendChild(e2);}};/* D1: context menu detail item removed */const ann=mkItem(ICON.annotate,""+(item.annotation?"编辑批注":"添加批注"),"A",false);ann.onclick=()=>{hideCtxMenu();state.selected=item.id;openAnnotation();};
