...
/* D1: Detail inline editor — reuses noteEditor + noteFormatBar */
...
function dockContent(st){
  ...
  if(st==="select"&&sel){
    ...
    if(sel.type==="mindLink"){
      ...
      out.push(dkSep());
      out.push(dkBtn({icon:ICON.edit,label:"编辑",title:"编辑内容 (双击)",fn:()=>{if(sel.type==="note")openTextEditor(sel);else{state.selected=sel.id;/* D1: openDetail removed */}}}));
    }else{
      out.push(dkBtn({icon:ICON.focus,label:"聚焦",title:"聚焦 (F)",fn:()=>{state.selected=sel.id;enterFocus(sel.id);}}));
      out.push(dkBtn({icon:ICON.annotate,label:"批注",title:"批注 (A)",fn:()=>openAnnotation()}));
    }
...
/* D1: dp-t/dp-sel/detailTextarea handlers all removed */
/* 批注：A键，选中元素后添加常驻小字 */
...
function hitNodeControl(wx,wy){
  ...
  return null;
}/* D1: detail keydown listener removed *//* 连接待选态：点击"连接"按钮时若仅选中 1 个元素，
