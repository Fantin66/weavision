...
function dockContent(st){
  ...
  if(st==="select"&&sel){
    ...
    if(sel.type==="mindLink"){
      out.push(dkLabel("层级连接"));
      out.push(dkBtn({icon:ICON.connector,label:"已选中",title:"层级连接已纳入统一的可选连接对象",on:true,fn:()=>{}}));
      ...
      out.push(dkSep());
      out.push(dkBtn({icon:ICON.edit,label:"编辑",title:"编辑内容 (双击)",fn:()=>{if(sel.type==="note")openTextEditor(sel);else{state.selected=sel.id;openDetail();}}}));
    }else{
      out.push(dkBtn({icon:ICON.focus,label:"聚焦",title:"聚焦 (F)",fn:()=>{state.selected=sel.id;enterFocus(sel.id);}}));
      out.push(dkBtn({icon:ICON.annotate,label:"批注",title:"批注 (A)",fn:()=>openAnnotation()}));
    }
...
function renderDockSub(){
  ...
  if(dockSubKind==="mdHeading"||dockSubKind==="mdBlock"){
    ...
    const grid=document.createElement("div");grid.className="ds-grid";
    var styleThumbs={
      clear:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px;background:#fff"><div style="flex:1;background:rgba(45,95,211,.12);border:1px solid rgba(45,95,211,.3);border-radius:8px"></div><div style="flex:1;background:rgba(25,161,135,.1);border:1px solid rgba(25,161,135,.25);border-radius:8px"></div></div>',
      glass:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px;background:linear-gradient(135deg,#e3f2fd,#fce4ec)"><div style="flex:1;background:rgba(255,255,255,.45);backdrop-filter:blur(4px);border:1px solid rgba(255,255,255,.5);border-radius:12px;box-shadow:0 2px 8px rgba(45,95,211,.08)"></div><div style="flex:1;background:rgba(255,255,255,.35);border:1px solid rgba(255,255,255,.4);border-radius:12px"></div></div>',
      minimal:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px"><div style="flex:1;background:transparent;border:1px solid rgba(0,0,0,.12);border-radius:2px"></div><div style="flex:1;background:transparent;border:1px solid rgba(0,0,0,.08);border-radius:2px"></div></div>',
      neumorph:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px;background:#eef0f4"><div style="flex:1;background:#eef0f4;box-shadow:3px 3px 6px rgba(0,0,0,.1),-3px -3px 6px rgba(255,255,255,.9);border-radius:12px"></div><div style="flex:1;background:#eef0f4;box-shadow:inset 2px 2px 4px rgba(0,0,0,.08),inset -2px -2px 4px rgba(255,255,255,.8);border-radius:10px"></div></div>',
      colorful:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px;background:#f2f3f7"><div style="flex:1;background:linear-gradient(135deg,#367bf0,#265AB1);box-shadow:2px 2px 4px rgba(0,0,0,.1),-2px -2px 4px rgba(255,255,255,.6);border-radius:12px"></div><div style="flex:1;background:linear-gradient(135deg,#19A187,#0d6b54);box-shadow:2px 2px 4px rgba(0,0,0,.1),-2px -2px 4px rgba(255,255,255,.6);border-radius:12px"></div></div>',
      bento:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px;background:#f8f9fb"><div style="flex:1.2;background:#ff6b6b;box-shadow:0 4px 12px rgba(0,0,0,.08);border-radius:18px"></div><div style="flex:1;background:#4ecdc4;box-shadow:0 2px 8px rgba(0,0,0,.04);border-radius:18px"></div></div>',
      editorial:'<div style="width:100%;height:100%;display:flex;flex-direction:column;gap:3px;padding:4px;background:#f8f9fb"><div style="flex:1;background:#ffe66d;border:1px solid rgba(0,0,0,.1);border-radius:4px"></div><div style="flex:1;background:#a8e6cf;border:1px solid rgba(0,0,0,.06);border-radius:4px"></div></div>',
    };
...
/* 旧版详情 Markdown 渲染器保留作兼容参考。
   实际预览统一使用后方的完整 mdToHtml，避免同名函数被静默覆盖。 */
...
function legacyDetailMdToHtml(src){
...
function openDetail(){
  ...
  /* 编辑窗不应改变画布几何；仅在真正提交内容后才按新内容形变。 */
  detailEditSession={id:s.id,initial:s.detail||"",wasExpanded:detailItemId===s.id};
  detailEditId=s.id;
...
function repairStaleDetailGeometry(){
  ...
  for(const it of state.items){
    ...
    if(detailLayout&&detailLayout.id===it.id)continue;
...
function syncDetailReadDom(){
  ...
  const it=detailItemId!==null&&state.items.find(x=>x.id===detailItemId);
  if(!it||!it.detail){detailReadLayer.innerHTML="";return;}
  const b=itemBounds(it);if(!b){detailReadLayer.innerHTML="";return;}
  const baseH=detailBaseHeight(it,b),bodyH=b.h-baseH;
  if(bodyH<4){detailReadLayer.innerHTML="";return;}
  let el=detailReadLayer.firstElementChild;
  if(!el){el=document.createElement("div");el.className="detail-read";detailReadLayer.appendChild(el);}
  /* 只在内容改变时重建，缩放/平移只更新几何，保留浏览器排版结果。 */
  if(el.dataset.itemId!==String(it.id)||el.dataset.source!==it.detail){
    el.dataset.itemId=String(it.id);el.dataset.source=it.detail;
  ...
  /* Markdown 的真实高度（尤其是表格、引用、换行）不能只靠字符数估算。
     只校正一次，再让节点连同阅读区一起长高，避免隐藏式滚动造成内容被截断。 */
  const layout=detailLayout;
  const measureKey=it.id+":"+it.detail+":"+Math.round(b.w);
  if(layout&&layout.id===it.id&&!layout.closing&&layout.measureKey!==measureKey){
    layout.measureKey=measureKey;
    requestAnimationFrame(()=>{
      if(detailLayout!==layout||layout.closing||detailItemId!==it.id)return;
      ...
      const neededW=Math.min(560,Math.max(layout.width||b.w,Math.ceil(body.scrollWidth||0)+4,tableWide));
      const needed=Math.ceil(body.scrollHeight||0)+2;
      if(needed<=bodyH+2&&neededW<=b.w+2)return;
      layout.width=Math.max(layout.width||0,neededW);
      layout.contentH=Math.max(layout.contentH||0,needed);
      const from=captureDetailGeometry();
      const to=makeExpandedGeometry(it,layout.backup,layout.baseH,layout.contentH,layout.width);
      animateDetailGeometry(from,to);
      if(state.focusMode&&state.focusMode.id===it.id){
        animateCamera(
          b.x+layout.width/2-W/2/state.camera.zoom,
          b.y+(layout.baseH+layout.contentH)/2-H/2/state.camera.zoom,
          state.camera.zoom
        );
      }
    });
...
function drawLinks(scope){
  ...
  for(const l of state.links){
    ...
    /* 关系线与导图层级线共用同一条“织线”规则：同样的路径、笔触和箭头；
       关系类型只通过颜色与虚实编码，避免看起来像两套不相干的功能。 */
    const LK=styleCfg();
    ctx.globalAlpha=active?.94:(LK.glass?.28:.22);
    /* 线型编码：关联/支撑=实线，导致=粗实线，反证=虚线，证据=点划线 */
    const baseW=(l.relationType==="causes"?2.5:1.9)*(LK.linkWidth||1);
