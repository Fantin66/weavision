...
function ensureMorphDom(it){
  ...
  const applyZoom=value=>{
    ...
    el.dataset.pvZoom=String(zoom);body.style.zoom=f.kind==="link"?"1":(f.kind==="doc"?"1":String(zoom));
    if(f.kind==="doc"){el.dataset.pvZoom=String(zoom);el.dataset.docxFitKey="";fitDocxPreview(el);}
...
function fitDocxPreview(bodyEl){
  ...
  var pvZoom=parseFloat(bodyEl.dataset.pvZoom)||1;var scale=clamp(available/nativeWidth*pvZoom,.12,3);
