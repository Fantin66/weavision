...
function renderFolderItem(folder){
  ...
  head.addEventListener("contextmenu",e=>{
    ...
    showOptions("文件夹「"+folder.name+"」",[
      {id:"child",label:"新建子文件夹",desc:"在此文件夹下继续整理",icon:ICON.folder,onClick:()=>showPrompt("新建子文件夹","文件夹名称","新文件夹",name=>{if(name)createFolder(name,folder.id);})},
      {id:"rename",label:"重命名",desc:"修改文件夹名称",icon:ICON.edit,onClick:()=>showPrompt("重命名文件夹","文件夹名称",folder.name,name=>{if(name){folder.name=name;renderFileGroups();saveState();}})},
      {id:"delete",label:"删除文件夹",desc:"文件会移到未分类，不会丢失",icon:ICON.trash,onClick:()=>removeFolder(folder.id)}
    ]);
  });
  wrap.appendChild(head);
  /* 子文件夹 */
...
function renderFileItem(f){
  ...
  item.addEventListener("contextmenu",ev=>{ev.preventDefault();ev.stopPropagation();showMoveFile(f);});
