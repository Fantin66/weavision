...
/* C13: Simplified detailBaseHeight — detail reading area removed, base = full height */
function detailBaseHeight(it,b){return b?b.h:(it?it.h||40:40);}
"use strict";
/* C4 Neumorphism */
...
function render(){
  ...
  /* C13: repairStaleDetailGeometry removed */
...
function drawNote(it,cc){
  ...
  /* （已移除：关联到节点时便签右上角的橙色角标） */ drawAnnotation(it,{x,y,w,h});
}
/* ---------- 画笔 ---------- */
...
function drawMindNode(it,cc){
  ...
  /* C13: detailToggleBounds removed */
  ...
  /* （已移除：节点的附件数量角标） */
  c.textBaseline="alphabetic";c.textAlign="start"; drawAnnotation(it,b);
}
/* 右侧的功能把手：展开内容与批注/跃迁信息 */function mindCollapseBounds(it,b){
...
function drawAnnotation(it,b){
  const z=state.camera.zoom;
  ctx.save();
  /* 有展开内容：嵌入节点右缘的竖向把手——低调中性，仅小文档图标，无背景色块（与批注纯文字风格统一） *//* C13: detail handle removed *//* 有跃迁：提供可点击的明确入口，而不是只展示目的地提示。 */
  if(it.jumpTo){
...
function drawFileCard(it,cc){
  ...
  c.textAlign="start";c.textBaseline="alphabetic"; drawAnnotation(it,b);
}
...
function enterFocus(id){
  ...
  /* C13: detail auto-expand removed */
