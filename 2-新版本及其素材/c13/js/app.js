...
function applyTheme(){
  ...
  /* C13: refresh logo on theme change */
...
document.addEventListener("keydown",e=>{
  ...
  /* E 键：编辑展开内容 */
  /* C13: 展开内容功能已移除（原 E 键 openDetail） */
  /* F 键：聚焦模式 */
...
function mountSearch(){
  ...
  function runSearch(){
    ...
    for(const it of state.items){
      ...
      if(label.toLowerCase().includes(q)||false /* C13: detail search removed */){
        results.push(it.id);
      }
    }
...
function init(){
  ...
  /* C13: load saved logo preference */
