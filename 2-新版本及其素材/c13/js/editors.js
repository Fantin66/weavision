...
function dockContent(st){
  ...
  if(st==="select"&&sel){
    ...
    if(sel.type==="mindLink"){
      ...
      out.push(dkSep());
      out.push(dkBtn({icon:ICON.edit,label:"编辑",title:"编辑内容 (双击)",fn:()=>{if(sel.type==="note")openTextEditor(sel);else{state.selected=sel.id;/* C13: openDetail removed — fileCard edit via double-click */}}}));
    }else{
      out.push(dkBtn({icon:ICON.focus,label:"聚焦",title:"聚焦 (F)",fn:()=>{state.selected=sel.id;enterFocus(sel.id);}}));
      out.push(dkBtn({icon:ICON.annotate,label:"批注",title:"批注 (A)",fn:()=>openAnnotation()}));
    }
...
function dockTargetState(){
  if(editingNoteId!==null||editingMindId!==null)return "edit";
...
/* ============================================================
   展开内容：Markdown 工具栏
============================================================ */
const dpPreview=null; /* C13: detailPanel removed */
...
/* C13: dp-t toolbar, dp-sel listener, and detail preview input handler all removed */
/* 批注：A键，选中元素后添加常驻小字 */
...
const mdMask=document.getElementById("mdModalMask");/* 展开是临时的“形变布局”，不写入用户的原始排布或撤销记录。 *//* 展开内容字体：正文用宋体系（与标题 Sans 字族区分），
   字号略小于标题（标题约 14-15，正文 13），保证美观与层级 */
const DETAIL_SERIF='"Noto Serif SC","Source Han Serif SC","Songti SC",Georgia,serif';
function detailFont(ln){return(ln.h===1?"700 14px ":(ln.h===2?"600 13px ":(ln.h===3?"600 12px ":"13px ")))+DETAIL_SERIF;}function movableDetailItems(){return state.items.filter(it=>it.type==="note"||it.type==="mindNode"||it.type==="fileCard");}function detailRectsOverlap(a,b,gap){
  return a.x<b.x+b.w+gap&&a.x+a.w>b.x-gap&&a.y<b.y+b.h+gap&&a.y+a.h>b.y-gap;
}/* 绘制展开内容：作为元素下半部分的延展阅读区，而不是外挂卡片。 *//* 检测点击是否命中展开把手或展开层 */function hitMindCollapse(it,b,wx,wy){
...
/* 控件要先于连接点命中：否则点击把手会被当作拖拽父子连线。 *//* C13: detail keydown listener removed */
