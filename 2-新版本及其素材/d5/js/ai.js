...
function aiBuildCanvas(plan){
  ...
  for(const spec of notes){
    ...
    note.annotation=spec.annotation||""; /* D5: note.detail removed — mind-node only */
  ...
  for(const spec of attachments){
    ...
    card.annotation=spec.annotation||""; /* D5: card.detail removed — mind-node only */
...
function aiExecute(command){
  try{
    ...
    switch(op){
      ...
      case "create_note":aiActivate(command.projectId||state.activeProjectId,command.canvasId||state.activeCanvasId);pushHistory();value=addNote(command.x||0,command.y||0,command.markdown||command.text||"",command.color||state.noteColor); /* D5: note.detail removed */aiCommit();break;
