...
function renameProject(id,name){
  ...
  if(p&&name){p.name=name;renderSidePanel();saveState();}
}
function renameFile(id,name){
  const f=state.files.find(x=>x.id===id);
  if(f&&name){f.name=name;renderFileGroups();saveState();}
}
...
const cloneCanvasState=()=>{const d={items:state.items,links:state.links,camera:state.camera,previews:state.previews};return typeof structuredClone==="function"?structuredClone(d):JSON.parse(JSON.stringify(d));};
...
function requestRender(){
  ...
  renderQueued=true;
  requestAnimationFrame(()=>{renderQueued=false;try{render();}catch(e){console.error("D5 RENDER ERROR:",e.message);}});
}
