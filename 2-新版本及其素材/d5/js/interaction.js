...
canvas.addEventListener("dblclick",e=>{
  ...
  const nc=hitNodeControl(wpt.x,wpt.y);if(nc&&nc.kind==="detail"&&nc.hit==="body"){state.selected=nc.item.id;openDetailEditor(nc.item);return;}if(hit&&(hit.type==="note"||hit.type==="mindNode")){ state.selected=hit.id; openTextEditor(hit); render(); }
    ...
    if(state.tempTool==="note"){
      pushHistory("添加便签");const n=addNote(wpt.x,wpt.y);state.selected=n.id;render();saveState();state.tempTool="";renderToolOptions();
    }else{
      /* 默认建导图节点（不管是不是 M 工具） */
...
board.addEventListener("wheel",e=>{
  ...
  if(e.ctrlKey){
    ...
    const isTrackpad=e.deltaMode===0&&(ax>.01||ay<88||Math.abs(ay%100)>1||now<trackpadPanUntil);
    if(isTrackpad){
  ...
  for(const mid of ids){
    ...
    if(mi&&mi.type==="mindNode"){
      const collectKids=node=>{for(const cid of node.children||[]){
        allIds.add(cid);const child=state.items.find(i=>i.id===cid);if(child)collectKids(child);
      }};
      collectKids(mi);
    }
  }
  ...
  if(hit){
    ...
    if(state.tool==="marquee"){
      ...
      if(e.ctrlKey||e.metaKey){
        ...
        render();renderDock(true);saveState();return;
      }
      if(idx<0){
        state.multiSel.push(hit.id);
        state.selected=hit.id;
        render();renderDock(true);saveState();
        return;
      }
      state.selected=hit.id;
...
function onPointerUp(e){
  ...
  /* 点击（无位移）文件卡片：
    ...
    renderDock(true);render();saveState();
    ...
    pushHistory("添加便签");var nn=addNote(wp.x,wp.y);state.selected=nn.id;render();saveState();
...
async function importFiles(fileList,folderId){
  ...
  for(const file of files){
    ...
    const f={id:"f"+(uid++),name:file.name,kind,size:file.size,mime:file.type,created:Date.now(),thumb:null,tw:1,th:1,folderId:actualFolderId||null,localPath:file.path||null};
    state.files.push(f);
    /* 存 blob */
    if(kind!=="link"){
      try{
...
function moveLibraryFile(fileId,folderId){
  ...
  file.folderId=folderId||null;renderFileGroups();saveState();
  toast("已移动到"+(folderId?"「"+folderPathName(state.folders.find(f=>f.id===folderId))+"」":"「未分类」"));
}
function showMoveFile(file){
  const opts=[{id:"root",label:"未分类",desc:"移动到资源库根级",icon:ICON.folder,onClick:()=>moveLibraryFile(file.id,null)}];
  for(const folder of state.folders)opts.push({id:folder.id,label:folderPathName(folder),desc:"移动到此文件夹",icon:ICON.folder,onClick:()=>moveLibraryFile(file.id,folder.id)});
  showOptions("移动「"+file.name+"」",opts);
}
/* 删除文件夹（文件移到根级） */
...
function renderSidePanel(){
  ...
  for(const p of state.projects){
    ...
    el.addEventListener("contextmenu",e=>{
      ...
      showFloatMenu(el,[
        {icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>',label:"重命名",onClick:()=>showPrompt("重命名项目","项目名称",p.name,function(name){if(name)renameProject(p.id,name);})},
        {sep:true},
  ...
  for(const c of cp.canvases){
    ...
    el.addEventListener("contextmenu",e=>{
      e.preventDefault();e.stopPropagation();
      showFloatMenu(el,[
        {icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>',label:"重命名",onClick:()=>showPrompt("重命名画布","画布名称",c.name,function(name){if(name)renameCanvas(c.id,name);})},
        {sep:true},
...
function renderFileGroups(){
  ...
  actionBar.addEventListener("drop",e=>{
    ...
    else if(folderId){const folder=state.folders.find(f=>f.id===folderId);if(folder){folder.parentId=null;renderFileGroups();saveState();toast("文件夹已移到根级");}}
  });
  ...
  if(rootFiles.length){
    ...
    head.addEventListener("click",()=>{state._rootFilesCollapsed=!state._rootFilesCollapsed;head.classList.toggle("open",!state._rootFilesCollapsed);list.classList.toggle("collapsed",state._rootFilesCollapsed);saveState();});
...
function renderFolderItem(folder){
  ...
  head.addEventListener("click",()=>{folder.collapsed=!folder.collapsed;head.classList.toggle("open",!folder.collapsed);list.classList.toggle("collapsed",folder.collapsed);saveState();});
  ...
  head.addEventListener("drop",e=>{
    ...
    if(movedFolderId&&movedFolderId!==folder.id&&!folderContains(movedFolderId,folder.id)){
      const moved=state.folders.find(f=>f.id===movedFolderId);if(moved){moved.parentId=folder.id;folder.collapsed=false;renderFileGroups();saveState();toast("文件夹已移动");}
    }
  });
  /* 右键提供整理操作 */
  head.addEventListener("contextmenu",e=>{
    ...
    showFloatMenu(head,[
      ...
      {icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>',label:"重命名",onClick:()=>showPrompt("重命名文件夹","文件夹名称",folder.name,function(name){if(name){folder.name=name;renderFileGroups();saveState();}})},
      {sep:true},
...
function renderFileItem(f){
  ...
  item.addEventListener("contextmenu",ev=>{
      ...
      showFloatMenu(item,[
        ...
        {icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>',label:"重命名",onClick:()=>showPrompt("重命名","文件名称",f.name,function(name){if(name)renameFile(f.id,name);})},
        {icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>',label:"移动到…",onClick:()=>showMoveFile(f)},
        {sep:true},
...
function exportUserData(){
  ...
  toast("已导出 "+userProjects.length+" 个用户项目");
}
