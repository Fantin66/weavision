...
/* D5: Detail inline editor — reuses noteEditor + noteFormatBar */
...
function dockContent(st){
  ...
  if(st==="select"&&sel){
    ...
    if(sel.type==="mindLink"){
      /* D5: mind links also support level/shape — stored on child node */
      ...
      if(sel.detail)out.push(dkBtn({icon:ICON.trash,label:"删除展开",title:"删除展开内容 (Alt+E)",fn:()=>{pushHistory("删除展开内容");sel.detail="";collapseDetailInPlace();render();saveState();toast("已删除展开内容");}}));
      ...
      out.push(dkSep());
      out.push(dkBtn({icon:ICON.edit,label:"编辑",title:"编辑内容 (双击)",fn:()=>{if(sel.type==="note")openTextEditor(sel);else{state.selected=sel.id;/* D5: openDetail removed */}}}));
    }else if(sel.type==="link"){
    /* D5: link dock — level, shape, relation, annotate, focus */
    var lvlLabels={normal:"普通",emphasis:"强调",highlight:"醒目"};
...
/* D5: dp-t/dp-sel/detailTextarea handlers all removed */
/* 批注：A键，选中元素后添加常驻小字 */
...
const mdMask=document.getElementById("mdModalMask");function markdownCanvasText(text){
  return String(text||"")
    .replace(/!\[([^\]]*)\]\([^)]*\)/g,"$1")
    .replace(/\[([^\]]+)\]\([^)]*\)/g,"$1")
    .replace(/(\*\*|__)(.*?)\1/g,"$2")
...
function syncDetailReadDom(){
  ...
  if(!it){detailReadLayer.innerHTML="";return;}
  if(!it.detail){
    /* D5: Show "double-click to edit" placeholder when no detail content */
    ...
    el.dataset.itemId=String(it.id);el.dataset.source="";
    el.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#999;font-size:13px;font-family:var(--font)">double-click to edit</div>';
...
function hitNodeControl(wx,wy){
  ...
  return null;
}/* D5: detail keydown listener removed *//* 连接待选态：点击"连接"按钮时若仅选中 1 个元素，
...
function drawLinks(scope){
  ...
  for(const l of state.links){
    ...
    const LK=styleCfg();
    /* D5: line level — width is the primary distinguisher, NOT glow/halo */
    ...
    /* D5: higher levels use more vivid color */
    ...
    /* D5: highlight level — inner light line (thin lighter stroke on top of thick line) */
...
function hitRelationLink(wx,wy){
  ...
  for(let i=state.links.length-1;i>=0;i--){
    ...
    if((a&&a.type==="mindNode"&&!isMindNodeVisible(a))||(b&&b.type==="mindNode"&&!isMindNodeVisible(b)))continue;
    const curve=linkCurve(l);if(!curve)continue;
    /* D5: use correct control points based on direction (horizontal vs vertical S-curve) */
    var p1x,p1y,p2x,p2y;
    if(curve.horizontal){
