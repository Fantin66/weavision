...
function render(){
  ...
  if(typeof syncDetailReadDom==="function")syncDetailReadDom(); /* D5: 展开内容阅读层同步 */
...
function drawMindConnections(scope){
  ...
  for(const ch of nodes){
    ...
    if(grow<1){
      ...
      ctx.save();
      /* D5: mind link level — width primary, no halo */
      ...
      ctx.stroke();
      /* D5: highlight level — inner light line */
...
function drawMindNode(it,cc){
  ...
  /* D5: detail toggle is on LEFT side now — doesn't affect text width */
...
function drawAnnotation(it,b){
  ...
  if(it.detail){
    /* D5: left-side tab button — distinct color, prominent, sticks out from left edge */
...
/* D5: removed detail modal handlers */

/* D5: Detail state variables (restored after truncation) */
