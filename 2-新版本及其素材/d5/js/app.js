...
function applyTheme(){
  ...
  /* D5: refresh logo on theme change */
...
document.addEventListener("keydown",e=>{
  ...
  if(e.key==="e"&&!e.ctrlKey&&!e.metaKey&&!e.altKey){
    ...
    else if(s.type==="fileCard"){console.log("D5 E: calling togglePreviewMorph");togglePreviewMorph(s);}
    return;
  }
  /* D5: Alt+E = delete detail content (mindNode only) */
...
/* D5: Float menu — positioned next to anchor element, not at cursor */
...
function showFloatMenu(anchorEl,items){
  ...
  setTimeout(()=>{
    document.addEventListener("pointerdown",hideFloatMenu,{once:true});
  },0);
}
