...
/* ---------------- 4. 时间轴（横向） ----------------
   主流「时间轴」：一条横向主轴，一级节点沿轴均匀分布（表示时间/顺序），
   二级节点挂在对应一级节点下方。 */
function timelineLayout(){
  const root=layoutRoot();if(!root)return;
  const ks=kidsOf(root);
...
/* ---------------- 5. 括号图（向右，大括号） ----------------
   主流「括号图」：整体在左、局部分支向右，用大括号包裹的形式表现总分关系。
   布局上与逻辑图一致，但绘制时连接线为花括号（见 drawMindConnections 的 brace 分支）。 */
function braceLayout(){
  logicRightLayout();
}
/* 布局分发：按当前 layoutType 执行（radial/both 已删除） */
function applyAutoLayout(){
  for(const it of state.items){
    if(it.type==="mindNode"&&it.collapsed)it.collapsed=false;
  }
...
/* 兼容旧名：radialLayout / bothSidesLayout 已移除，指向新的分发 */
function radialLayout(){applyAutoLayout();}
function bothSidesLayout(){applyAutoLayout();}
/* 避免所有元素之间的遮挡：检测重叠并推开 */
