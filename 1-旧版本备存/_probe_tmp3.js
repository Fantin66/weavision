...
const LINE_COLORS=["#3a4a6b","#c94a3e","#e0882a","#1fa06a","#0f8aa0","#7a55c0","#9090a0"];
const MIND_COLORS=["#3a4a6b","#0f8aa0","#1fa06a","#7a55c0","#e0882a","#c94a3e"];
...
function render(){
  ...
  if(state.hover && state.hover.id&&(!sel||state.hover.id!==sel.id)){
  ...
  if(state.hover&&state.hover.type==="mindNode"){
    ...
    if(n&&n.attachIds&&n.attachIds.length){
      for(const aid of n.attachIds){
        const a=state.items.find(x=>x.id===aid);
        if(!a) continue;
        const ab=itemBounds(a);
        if(!ab) continue;
        ctx.save();
        ctx.strokeStyle="rgba(52,199,89,.7)";ctx.lineWidth=2/z;
        ctx.setLineDash([4/z,3/z]);
        roundRectPath(ctx,ab.x-3/z,ab.y-3/z,ab.w+6/z,ab.h+6/z,10);ctx.stroke();
        ctx.restore();
      }
    }
  }
  if(drag){
    /* 拖拽便签/材料时：所有节点轻微高亮提示可挂接 */
    if(drag.mode==="move"&&(drag.item.type==="note"||drag.item.type==="fileCard")){
      ctx.save();
      ctx.lineWidth=1.5/z;ctx.strokeStyle="rgba(58,74,107,.2)";ctx.setLineDash([4/z,3/z]);
      for(const it of state.items){
        if(it.type!=="mindNode"||it.id===drag.item.id)continue;
    ...
    if(drag.mode==="move"&&drag.attachTarget){
      const t=itemBounds(drag.attachTarget);
      ctx.save();
      ctx.lineWidth=2.5/z;ctx.strokeStyle="#2a7a6a";
      ctx.shadowColor="rgba(52,199,89,.5)";ctx.shadowBlur=12/z;
      ctx.setLineDash([6/z,4/z]);
      roundRectPath(ctx,t.x-4/z,t.y-4/z,t.w+8/z,t.h+8/z,10);ctx.stroke();
      /* 关联图标 */
      ctx.fillStyle="#2a7a6a";ctx.font=(12/z)+"px sans-serif";ctx.textAlign="center";
      ctx.fillText("⊕",t.x+t.w/2,t.y-8/z);
      ctx.restore();
    }
...
function drawNote(it,cc){
  ...
  c.save();
  c.shadowColor=sel?"rgba(58,74,107,.35)":"rgba(15,23,42,.13)";
  c.shadowBlur=(sel?10:6)/z;c.shadowOffsetY=2/z;
  ...
  if(sel){
    c.save();
    c.strokeStyle="rgba(58,74,107,.2)";c.lineWidth=4/z;
    roundRectPath(c,x-1.5/z,y-1.5/z,w+3/z,h+3/z,8.5);c.stroke();
    c.strokeStyle="rgba(58,74,107,.85)";c.lineWidth=1.5/z;
    roundRectPath(c,x+.8/z,y+.8/z,w-1.6/z,h-1.6/z,6.5);c.stroke();
    c.restore();
  }
  if(w>34&&h>22){
  ...
  /* 已关联到导图节点 => 右上角橙色小圆点+白环 */
  if(nodeOf(it)){
    c.save();
    c.fillStyle="#c48840";
    c.beginPath();c.arc(x+w-6,y+6,4.5/z,0,7);c.fill();
    c.strokeStyle="rgba(255,255,255,.7)";c.lineWidth=1/z;
    c.beginPath();c.arc(x+w-6,y+6,5.5/z,0,7);c.stroke();
    c.restore();
  }
...
function drawMindNode(it,cc){
  ...
  if(sel){
    ...
    /* 外发光 */
    c.strokeStyle="rgba(58,74,107,.2)";c.lineWidth=4/z;
    roundRectPath(c,b.x-1.5/z,b.y-1.5/z,b.w+3/z,b.h+3/z,r+1.5);c.stroke();
    /* 内描边 */
    c.strokeStyle="rgba(58,74,107,.85)";c.lineWidth=1.5/z;
    roundRectPath(c,b.x+.8/z,b.y+.8/z,b.w-1.6/z,b.h-1.6/z,r);c.stroke();
  ...
  /* 附件角标 */
  if(it.attachIds&&it.attachIds.length){
    const cnt=it.attachIds.length;
    const bx=b.x+b.w-8,by=b.y-2;
    c.save();
    c.fillStyle="#c48840";
    c.beginPath();c.arc(bx,by,9/z,0,7);c.fill();
    c.fillStyle="#fff";c.font="700 10px sans-serif";c.textAlign="center";c.textBaseline="middle";
    c.fillText(String(cnt),bx,by+0.5);
    c.restore();
  }
...
function drawFileCard(it,cc){
  ...
  if(sel){
    c.save();
    c.strokeStyle="rgba(58,74,107,.2)";c.lineWidth=4/z;
    roundRectPath(c,b.x-1.5/z,b.y-1.5/z,b.w+3/z,b.h+3/z,11.5);c.stroke();
    c.strokeStyle="rgba(58,74,107,.85)";c.lineWidth=1.5/z;
  ...
  if(nodeOf(it)){
    c.save();
    c.fillStyle="#c48840";
    c.beginPath();c.arc(b.x+b.w-7,b.y+7,4.5/z,0,7);c.fill();
    c.strokeStyle="#fff";c.lineWidth=1.5/z;
    c.beginPath();c.arc(b.x+b.w-7,b.y+7,5.5/z,0,7);c.stroke();
    c.restore();
  }
...
function linkCurve(l){
  ...
  const ea=edgePoint(ax,ay,bx,by,ab),eb=edgePoint(bx,by,ax,ay,bb);
  return{ea,eb,mx:(ea.x+eb.x)/2};
}
function strokeLinkCurve(c,curve){c.beginPath();c.moveTo(curve.ea.x,curve.ea.y);c.bezierCurveTo(curve.mx,curve.ea.y,curve.mx,curve.eb.y,curve.eb.x,curve.eb.y);}
function drawLinks(scope){
  ...
  for(const l of state.links){
    ...
    if(l.directional||rel.directional){
      const angle=Math.atan2(eb.y-ea.y,eb.x-mx),size=9/z;
...
function drawSelection(it){
  ...
  c.save();
  if(it.type==="mindNode"){
    /* mindNode 选中发光：在节点周围画一圈外发光 */
    c.shadowColor="rgba(58,74,107,.4)";c.shadowBlur=16/z;
    c.strokeStyle="rgba(58,74,107,.6)";c.lineWidth=2/z;c.setLineDash([]);
    roundRectPath(c,b.x-1/z,b.y-1/z,b.w+2/z,b.h+2/z,12);c.stroke();
    c.setLineDash([]);c.restore();
  }
  else{
    /* 便签/卡片选中：发光描边 + 半透明背景 */
    c.setLineDash([]);
    c.shadowColor="rgba(58,74,107,.35)";c.shadowBlur=14/z;
    c.lineWidth=2/z;c.strokeStyle="#3a4a6b";
    roundRectPath(c,b.x-1/z,b.y-1/z,b.w+2/z,b.h+2/z,Math.min(10,b.h/4));c.stroke();
    c.shadowColor="transparent";
    c.restore();
    if(it.type==="note"||it.type==="fileCard"){
      const r=3.3/z,inset=3.5/z;
      for(const h of HANDLES){
        const p=handlePos(b,h);
        p.x=clamp(p.x,b.x+inset,b.x+b.w-inset);p.y=clamp(p.y,b.y+inset,b.y+b.h-inset);
        c.fillStyle="#fff";c.strokeStyle="#3a4a6b";c.lineWidth=2/z;
...
/* ============================================================
   便签/材料 ↔ 导图节点 关联（attachIds）
============================================================ */
function attachToNode(nodeId,itemId){
  const n=state.items.find(i=>i.id===nodeId);
  const it=state.items.find(i=>i.id===itemId);
  if(!n||!it||n.type!=="mindNode") return false;
  if(it.type==="mindNode") return false; // 节点不挂节点
  pushHistory("关联挂接");
  if(!n.attachIds) n.attachIds=[];
  if(n.attachIds.includes(itemId)){ /* 已关联则取消 */ n.attachIds=n.attachIds.filter(x=>x!==itemId); toast("已取消关联"); }
  else{ n.attachIds.push(itemId); toast("已关联到「"+n.text+"」"); }
  render();saveState();
  return true;
}
...
function attachedItems(n){
  return (n.attachIds||[]).map(id=>state.items.find(i=>i.id===id)).filter(Boolean);
}
function nodeOf(item){
  return state.items.find(n=>n.type==="mindNode"&&(n.attachIds||[]).includes(item.id))||null;
}
...
function autoLayout(){
  ...
  if(state.layoutType==="radial"){radialLayout();avoidOverlap();return;}
  if(state.layoutType==="both"){bothSidesLayout();avoidOverlap();return;}
...
/* 居中发散布局：真正的爆炸形构图，按子树规模分配角度 */
function radialLayout(){
  const root=state.items.find(it=>it.type==="mindNode"&&!it.parentId);
  if(!root)return;
  const rb=itemBounds(root);
  root.x=-rb.w/2;root.y=-rb.h/2;
  const children=(root.children||[]).map(id=>state.items.find(it=>it.id===id)).filter(Boolean);
  if(!children.length)return;
  const sector=2*Math.PI/children.length;
  /* 计算每个子树的叶节点数，按比例分配角度 */
  function leafCount(n){
    if(!n.children||!n.children.length||n.collapsed)return 1;
    return n.children.reduce((s,cid)=>{const c=state.items.find(i=>i.id===cid);return s+(c?leafCount(c):1);},0);
  }
  function placeBranch(node,angle,depth,span){
    const b=itemBounds(node);
    /* 半径根据深度递增，但不同子树可以不同步长 */
    const radius=180+depth*160;
    node.x=Math.cos(angle)*radius-b.w/2;
    node.y=Math.sin(angle)*radius-b.h/2;
    if(node.collapsed)return;
    const kids=(node.children||[]).map(id=>state.items.find(it=>it.id===id)).filter(Boolean);
    if(!kids.length)return;
    /* 子节点角度按叶节点数加权分配 */
    const childSpan=Math.min(span*.75,Math.PI*.6);
    const totalLeaves=kids.reduce((s,k)=>s+leafCount(k),0);
    let accAngle=angle-childSpan/2;
    kids.forEach(kid=>{
      const lc=leafCount(kid);
      const kidSpan=childSpan*lc/totalLeaves;
      const kidAngle=accAngle+kidSpan/2;
      placeBranch(kid,kidAngle,depth+1,kidSpan);
      accAngle+=kidSpan;
    });
  }
  children.forEach((child,i)=>placeBranch(child,-Math.PI/2+i*sector,1,sector));
}
/* 两侧分布布局 */
function bothSidesLayout(){
  const root=state.items.find(it=>it.type==="mindNode"&&!it.parentId);
  if(!root)return;
  const rb=itemBounds(root);
  root.x=-rb.w/2;root.y=-rb.h/2;
  const branches=(root.children||[]).map(id=>state.items.find(it=>it.id===id)).filter(Boolean);
  const GAP_Y=24,GAP_X=82;
  function branchHeight(node){
    const b=itemBounds(node),kids=(node.children||[]).map(id=>state.items.find(it=>it.id===id)).filter(Boolean);
    if(node.collapsed||!kids.length)return b.h;
    return Math.max(b.h,kids.reduce((sum,k)=>sum+branchHeight(k),0)+GAP_Y*(kids.length-1));
  }
  /* 按子树高度动态配平左右两翼，而非机械地切成两半。 */
  const left=[],right=[];let leftH=0,rightH=0;
  [...branches].sort((a,b)=>branchHeight(b)-branchHeight(a)).forEach(branch=>{
    const h=branchHeight(branch);
    if(leftH<=rightH){left.push(branch);leftH+=h+GAP_Y;}else{right.push(branch);rightH+=h+GAP_Y;}
  });
  function placeBranch(node,side,centerX,centerY){
    const b=itemBounds(node);node.x=centerX-b.w/2;node.y=centerY-b.h/2;
    if(node.collapsed)return;
    const kids=(node.children||[]).map(id=>state.items.find(it=>it.id===id)).filter(Boolean);
    if(!kids.length)return;
    const total=kids.reduce((sum,k)=>sum+branchHeight(k),0)+GAP_Y*(kids.length-1);
    let cursor=centerY-total/2;
    for(const kid of kids){
      const kb=itemBounds(kid),kh=branchHeight(kid);
      const nextX=centerX+side*(b.w/2+GAP_X+kb.w/2);
      placeBranch(kid,side,nextX,cursor+kh/2);cursor+=kh+GAP_Y;
    }
  }
  function placeSide(list,side){
    const total=list.reduce((sum,n)=>sum+branchHeight(n),0)+GAP_Y*Math.max(0,list.length-1);
    let cursor=-total/2;
    for(const n of list){
      const b=itemBounds(n),h=branchHeight(n);
      const centerX=side*(rb.w/2+GAP_X+b.w/2);
      placeBranch(n,side,centerX,cursor+h/2);cursor+=h+GAP_Y;
    }
  }
...
/* 局部避让：只推开与 dragged 元素重叠的其他元素，不改变其他元素之间的位置 */
...
function showNoteEditorChrome(it,tl,br){
  ...
  const editorW=br.x-tl.x,editorH=br.y-tl.y,previewW=clamp(Math.max(220,editorW*1.1),220,360),previewH=Math.max(140,editorH),gap=12;
  notePreview.style.display="block";
  notePreview.style.width=previewW+"px";notePreview.style.height=previewH+"px";
  let px=br.x+gap,py=tl.y;
  if(px+previewW>W-8){
    px=tl.x-previewW-gap;
    if(px<8){
      px=clamp(tl.x,8,Math.max(8,W-previewW-8));py=br.y+gap;
      if(py+previewH>H-8)py=Math.max(8,tl.y-previewH-gap);
    }
  }
  notePreview.style.left=px+"px";notePreview.style.top=py+"px";
  updateNotePreview();
}
...
function onPointerDown(e){
  ...
  if(hit){
    ...
    /* Ctrl/Cmd+点击：多选，第一个=父项，后续=子项 */
    if(e.ctrlKey||e.metaKey){
      const idx=state.multiSel.indexOf(hit.id);
      if(idx>=0){ state.multiSel.splice(idx,1); }
      else{ state.multiSel.push(hit.id); state.selected=hit.id; }
      render();return;
    }
    /* 非 Ctrl：单选，清空多选 */
...
function onPointerMove(e){
  ...
  if(drag.mode==="move"){
    ...
    /* 便签/材料拖到导图节点上 */
    if(it.type==="note"||it.type==="fileCard"){
      drag.attachTarget=mindDropTarget(wpt.x,wpt.y,it.id);
      render();return;
    }
    render();return;
  }
...
function onPointerUp(e){
  ...
  else if(d.mode==="move"&&d.attachTarget){
    /* 便签/材料拖到节点上，松手 => 关联 */
    attachToNode(d.attachTarget.id,d.item.id);
    render();
  }
  else if(d.mode==="move"&&d.item.type==="fileCard"&&d.movedDist<6){
    /* 点击（无位移）文件卡片 => 画布内预览 */
    openPreview(d.item.fileId);
  }
...
function loadState(){
  try{
    ...
    if(d.projects&&d.projects.length){
      ...
      const COLOR_MIGRATE={"#2a8a9a":"#0f8aa0","#4a9a5a":"#1fa06a","#8a5a9a":"#7a55c0","#d48840":"#e0882a","#c5483a":"#c94a3e","#fde68a":"#fef3c7","#dcfce7":"#dbeafe","#f5e6c8":"#fef3c7","#f0d690":"#fef3c7","#e8c5c0":"#fde2e2","#c5c0e0":"#ede4ff","#b8c8e0":"#dbeafe","#a8d0c0":"#ccfbf1","#e8e8ea":"#f5f5f4","#4a9a5a":"#1fa06a","#7a7a82":"#9090a0"};
      if(COLOR_MIGRATE[noteColor])state.noteColor=COLOR_MIGRATE[state.noteColor];
...
document.addEventListener("keydown",e=>{
  if(isTyping()){
    ...
    if(e.key==="Enter"&&!e.shiftKey){
      ...
      else if(editingNoteId!==null) closeEditor(false);
    }
    return;
  }
...
function buildFilterBar(){
  ...
  /* 快速操作按钮 */
  const actions=[
    {icon:ICON.plus,label:"节点",fn:()=>{const n=addMindNode("新节点",null,state.mindColor,state.camera.x+100,state.camera.y+100);smartPlace(n);state.selected=n.id;render();saveState();toast("已添加节点");}},
    {icon:ICON.note,label:"便签",fn:()=>{const n=addNote(state.camera.x+100,state.camera.y+100);smartPlace(n);state.selected=n.id;render();saveState();}},
    {icon:ICON.pen,label:"画笔",fn:()=>{state.tempTool=state.tempTool==="pen"?"":"pen";toast(state.tempTool==="pen"?"画笔模式":"已退出");}},
  ];
  actions.forEach((a,i)=>{
    if(i>0){const s=document.createElement("span");s.className="fsep";bar.appendChild(s);}
    const b=document.createElement("button");
    b.className="fbtn";
  ...
  /* 分隔线 */
  const sep=document.createElement("span");sep.className="fsep";bar.appendChild(sep);
  /* 筛选按钮 */
  FILTERS.forEach((f,i)=>{
    const s=document.createElement("span");s.className="fsep";bar.appendChild(s);
    const b=document.createElement("button");
    b.className="fbtn"+(state.filter===f.id?" on":"");
    b.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M7 12h10M10 18h4"/></svg><span>'+f.label+'</span>';
...
function mountControls(){
  ...
  const MENUS={
    layout:[
      ...
      {sep:true},
      {icon:"",label:"向右树形",key:"",fn:()=>{state.layoutType="right";autoLayout();render();saveState();toast("向右树形");}},
      {icon:"",label:"居中发散",key:"",fn:()=>{state.layoutType="radial";radialLayout();render();saveState();toast("居中发散");}},
      {icon:"",label:"两侧分布",key:"",fn:()=>{state.layoutType="both";bothSidesLayout();render();saveState();toast("两侧分布");}},
    ],
    add:[
