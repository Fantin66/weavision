...
============================================================ */
const STYLE_PRESETS={
  clear:{label:"清晰",desc:"扁平轻投影，信息密度高"},
  glass:{label:"玻璃拟态",desc:"半透明磨砂玻璃，通透悬浮"},
  paper:{label:"拟物纸感",desc:"半透明便利贴 + 钢笔手绘连线"},
  soft:{label:"新拟态",desc:"柔和浮雕，单色微质感"},
};
...
function styleCfg(){
  ...
  if(s==="paper")return{
    id:"paper",
    nodeAlpha:.88,          /* 纸张微透（比玻璃实） */
    nodeBlur:0,
    borderAlpha:dark?.26:.3, /* 纸边：可见实线 */
    borderColor:dark?"rgba(255,255,255,.36)":"rgba(60,45,20,.35)",
    shadowBlur:dark?10:8, shadowAlpha:dark?.42:.3,
    innerGlow:dark?.08:.16, /* 纸不反光 */
    radius:4,               /* 纸感：近直角小圆角 */
    linkStyle:"pen",        /* 连线：钢笔手绘质感 */
    linkAlpha:.85,
    linkWidth:1.15,         /* 笔触更粗 */
    fontScale:1.03,         /* 字号略大，纸质阅读感 */
    outlined:true,
    paper:true,             /* 纸张专属：纸纹+卷角 */
  };
  if(s==="soft")return{
    id:"soft",
    nodeAlpha:1,            /* 完全不透明，纯浮雕 */
    nodeBlur:0,
    borderAlpha:0,          /* 无描边，靠光影区分 */
    borderColor:"rgba(0,0,0,0)",
    shadowBlur:dark?18:16, shadowAlpha:dark?.55:.42,  /* 最强阴影=浮雕 */
    innerGlow:dark?.2:.5,   /* 强内高光=凸起 */
    radius:18,              /* 最圆 */
    linkStyle:"clean",
    linkAlpha:.4,           /* 连线最淡 */
    linkWidth:.9,
    fontScale:1,
    outlined:false,
    soft:true,              /* 新拟态专属：双向阴影（凸起） */
  };
  return{ /* clear 默认 */
    id:"clear",
    nodeAlpha:1,
    nodeBlur:0,
    borderAlpha:dark?.16:.14,
    borderColor:dark?"rgba(255,255,255,.22)":"rgba(0,0,0,.16)",
    shadowBlur:dark?10:8, shadowAlpha:dark?.3:.14,  /* 最轻阴影=扁平 */
...
const state={
  ...
  layoutType:"right",    /* 布局形态：right/org/fishbone/timeline/brace */
...
function render(){
  ...
  if(focusId!==null){
    ...
    for(const it of state.items){
      if(focusPrimary.has(it.id)&&it.id!==focusId&&filterOk(it)){
        drawItem(it);
        /* 关联元素高亮：黄色高亮 + 发光（废弃深色实线） */
        const b=itemBounds(it);if(b){
          ctx.save();
          const HL="#f5c518";
          ctx.lineWidth=2/z;
          ctx.strokeStyle=HL;
          ctx.shadowColor="rgba(245,197,24,"+(state.dark?".65":".5")+")";
          ctx.shadowBlur=16/z;
    ...
    if(focusIt&&filterOk(focusIt)){
      ...
      if(fb){
        ...
        /* 第1层：柔和光晕（在元素下方，形成悬浮发光感）—— 黄色高亮 */
        ctx.save();
        const GLOW="#f5c518";           /* 品牌黄：聚焦高亮 */
        const GLOW_RGBA="rgba(245,197,24,";
        ctx.shadowColor=GLOW_RGBA+(state.dark?".5":".45")+")";
        ctx.shadowBlur=38/z;
        ctx.strokeStyle=GLOW;ctx.lineWidth=8/z;ctx.globalAlpha=.4;
        roundRectPath(ctx,gb.x-2/z,gb.y-2/z,gb.w+4/z,gb.h+4/z,14);ctx.stroke();
        ctx.restore();
        /* 绘制放大的元素本体 */
        ctx.save();
        ctx.translate(cx,cy);
        ...
        drawItem(focusIt);
        ctx.restore();
        /* 第2层：黄色高亮环（贴合放大后边界，不再小于元素）+ 内侧白光 */
        ctx.save();
        ctx.shadowColor="rgba(245,197,24,"+(state.dark?".55":".5")+")";
        ctx.shadowBlur=18/z;
        ctx.strokeStyle="#f5c518";ctx.lineWidth=3/z;
        roundRectPath(ctx,gb.x-3/z,gb.y-3/z,gb.w+6/z,gb.h+6/z,13);ctx.stroke();
        ctx.shadowColor="transparent";
        ctx.strokeStyle="rgba(255,255,255,.65)";ctx.lineWidth=1/z;
        roundRectPath(ctx,gb.x-1/z,gb.y-1/z,gb.w+2/z,gb.h+2/z,12);ctx.stroke();
        ctx.restore();
      }
    }
    ctx.restore();
    ...
    if(hov&&filterOk(hov)&&(hov.type!=="mindNode"||isMindNodeVisible(hov))){
      ...
      if(hb){
        ...
        if(hov.type==="mindNode"||hov.type==="note"){
          ctx.shadowColor=hov.type==="mindNode"?"rgba(45,95,211,.15)":"rgba(255,255,255,.3)";
          ctx.shadowBlur=14/z;
  ...
  if(linkAnimMap.size){
    ...
    for(const [lid] of linkAnimMap){
      if(lid.indexOf("mind:")===0){
        ...
        const pOk=state.items.some(i=>i.id===parts[1]);
        const cOk=state.items.some(i=>i.id===parts[2]);
...
/* ============================================================
   ...
   逻辑：iframe 的逻辑宽度 = 预览窗的世界宽度 pv.w（内容按窗口排版），
   物理显示 = 逻辑宽 * z（画布缩放），从而内容与画布其余元素等比联动；
   PC/手机模式切换仅影响逻辑宽(1280 / 375)，缩放同样跟随 z。
============================================================ */
function syncPvContentZoom(el,pv,z){
  if(!el||!pv)return;
  const body=el.querySelector(".pv-body");
  if(!body)return;
  /* 1) 网页链接（iframe）：逻辑视口固定（PC 1280 / 手机 375），
       整体 transform 等比缩放铺满容器 —— 内容不重排，始终按设计宽度排版 */
  const iframe=body.__pvIframe;
  const stage=body.__pvStage;
  if(iframe&&stage){
    const mb=body.__pvMb?body.__pvMb():false;
    const lw=mb?375:1280;                       /* 逻辑视口宽（排版基准，恒定） */
    const lh=mb?667:760;                        /* 逻辑视口高 */
    /* 可用世界宽：预览窗世界宽 - 两侧留白（挤空确保内容完整可见） */
    const availW=Math.max(60,pv.w-16);
    iframe.style.width=lw+"px";                 /* 恒为逻辑宽，不随容器变 */
    iframe.style.height=lh+"px";
    /* 缩放比 = 容器可用宽 / 逻辑宽 × 画布 zoom（内容与画布元素等比联动） */
    const ratio=(availW/lw)*z;
    iframe.style.transform="scale("+ratio+")";
    iframe.style.transformOrigin="top center";
    stage.style.height=Math.round(lh*ratio)+"px";
...
function syncMorphDom(){
  if(!morphDom)return;
  /* 防御：若节点已被其他逻辑移出 DOM（如旧版 syncPvDom 清孤儿），重新挂载 */
  if(!document.body.contains(morphDom)){
    const layer=document.getElementById("previewLayer");
    if(layer)layer.appendChild(morphDom);
    else{destroyMorphDom();return;}
  }
  const card=state.items.find(i=>i.type==="fileCard"&&String(i.id)===morphDom.dataset.card);
  if(!card||!card.previewOpen){
    destroyMorphDom();
    return;
  }
  const b=itemBounds(card);
  if(!b){destroyMorphDom();return;}
  const z=state.camera.zoom;
  const tl=w2s(b.x,b.y+PV_HEAD_H/z);
  const w=Math.round(b.w*z),h=Math.round((b.h-PV_HEAD_H/z)*z);
  morphDom.style.left=tl.x+"px";
  morphDom.style.top=tl.y+"px";
  morphDom.style.width=Math.max(20,w)+"px";
  morphDom.style.height=Math.max(20,h)+"px";
  /* 形变覆盖层内的 iframe（网页链接）同样随画布 zoom 等比缩放
     —— 与 syncPvContentZoom 同一公式：逻辑宽恒定，transform 等比 */
  const mBody=morphDom.querySelector?morphDom:null;
  if(mBody){
    const iframe=mBody.__pvIframe;
    const stage=mBody.__pvStage;
    if(iframe&&stage){
      /* 逻辑宽恒定（网页链接默认手机 375 / PC 1280），由 __pvMb 决定 */
      const mb=mBody.__pvMb?mBody.__pvMb():false;
      const lw=mb?375:1280,lh=mb?667:760;
      const availW=Math.max(60,b.w-16);
...
function drawGrid(z){
  ...
  const fine=dark?"rgba(120,140,180,.06)":"rgba(45,95,211,.04)";
  const bold=dark?"rgba(120,140,180,.1)":"rgba(45,95,211,.08)";
...
/* ③ 路线：主轴走 S 形贝塞尔；跨轴走圆角正交折线 */
function routeConnection(c,a,b,outSide,inpSide,color,width){
  const z=state.camera.zoom;
  const ax=Math.abs(b.x-a.x),ay=Math.abs(b.y-a.y);
  const horiz=(outSide==="l"||outSide==="r");
  c.beginPath();c.moveTo(a.x,a.y);
  if(horiz){
    /* 出边/入边都在左右 → S 形（控制点取水平中点） */
    const mx=(a.x+b.x)/2;
    c.bezierCurveTo(mx,a.y,mx,b.y,b.x,b.y);
  }else if((outSide==="t"||outSide==="b")&&(inpSide==="t"||inpSide==="b")){
    /* 上下 → S 形（控制点取垂直中点） */
    const my=(a.y+b.y)/2;
    c.bezierCurveTo(a.x,my,b.x,my,b.x,b.y);
  }else{
    /* 跨轴（如 右→上）：圆角正交折线，拐角半径自适应 */
    const r=Math.min(14/z,ax/2,ay/2);
    if(outSide==="r"||outSide==="l"){
      const sx=outSide==="r"?r:-r;
      const ey=inpSide==="t"?-r:r;
      c.lineTo(b.x+sx,a.y);
      c.quadraticCurveTo(b.x,a.y,b.x,a.y+ey);
      c.lineTo(b.x,b.y);
    }else{
      const sy=outSide==="b"?r:-r;
      const ex=inpSide==="l"?-r:r;
      c.lineTo(a.x,b.y+sy);
      c.quadraticCurveTo(a.x,b.y,a.x+ex,b.y);
      c.lineTo(b.x,b.y);
    }
  }
}
...
function drawMindConnections(scope){
  ...
  for(const ch of nodes){
    ...
    let pa=anchorOn(pb,sides.out),cb0=anchorOn(chb,sides.inp);
    /* 平滑过渡：拖动的每一帧对端点/朝向做指数平滑（约90ms收敛），
       避免方位切换时连线生硬跳变；自动布局时已清动画状态=定格就位 */
    const sm=smoothLinkEndpoints({id:"mind:"+p.id+":"+ch.id},pa,cb0,sides.out,sides.inp);
    pa=sm.ea;cb=sm.eb;
    ...
    if(grow<1){
      ...
      /* 通用路线：主轴 S 形 / 跨轴圆角折线 */
      routeConnection(ctx,pa,cb,outSide,inpSide,g,(isSel?2.2:1.8)/z);
      ctx.stroke();
      ...
      ctx.stroke();
      /* 箭头沿入边法线，语义清晰 */
      ctx.globalAlpha=isSel?.9:.7;
      drawAnchorArrow(ctx,cb,inpSide,ch.color,8/z);
      ctx.restore();
...
function drawStroke(it,cc){
  ...
  if(sel){
    c.save();
    c.strokeStyle="rgba(45,95,211,.3)";c.lineWidth=it.size+3/z;
    c.lineCap="round";c.lineJoin="round";
    c.beginPath();it.points.forEach((p,i)=>i?c.lineTo(p.x,p.y):c.moveTo(p.x,p.y));c.stroke();
    c.restore();
  }
...
function drawMindNode(it,cc){
  ...
  else if(depth===0){c.shadowColor="rgba(0,0,0,"+(SC.shadowAlpha+.08)+")";c.shadowBlur=SC.shadowBlur/z;c.shadowOffsetY=4/z;}
  else if(depth===1){c.shadowColor="rgba(0,0,0,"+SC.shadowAlpha+")";c.shadowBlur=Math.max(6,SC.shadowBlur-3)/z;c.shadowOffsetY=2/z;}
  else if(depth===2){c.shadowColor="rgba(0,0,0,"+(SC.shadowAlpha*.6)+")";c.shadowBlur=Math.max(4,SC.shadowBlur-6)/z;c.shadowOffsetY=1.5/z;}
  else{c.shadowColor="transparent";}
  /* 圆角：样式基准 + 层级递减 */
  const rBase=SC.radius;
  const r=depth===0?rBase:depth===1?rBase-2:depth===2?rBase-4:rBase-5;
  ...
  /* 风格专属质感 */
  if(SC.soft){
    /* 新拟态：上方白内阴影 + 下方暗外阴影 = 凸起浮雕 */
    c.save();c.shadowColor="transparent";
    roundRectPath(c,b.x,b.y,b.w,b.h,r);c.clip();
    const ng=c.createLinearGradient(b.x,b.y,b.x,b.y+b.h);
    ng.addColorStop(0,"rgba(255,255,255,"+(state.dark?.16:.9)+")");
    ng.addColorStop(.5,"rgba(255,255,255,0)");
    ng.addColorStop(1,"rgba(0,0,0,"+(state.dark?.2:.06)+")");
    c.fillStyle=ng;c.fillRect(b.x,b.y,b.w,b.h);
    c.restore();
  }
  if(SC.glass){
    /* 玻璃拟态：斜向高光带 */
    ...
    gg.addColorStop(0,"rgba(255,255,255,.5)");
    gg.addColorStop(.45,"rgba(255,255,255,.06)");
  ...
  if(SC.paper){
    ...
    c.beginPath();
    c.moveTo(b.x+b.w,b.y+b.h-14);
    c.lineTo(b.x+b.w,b.y+b.h);
    c.lineTo(b.x+b.w-14,b.y+b.h);
    c.closePath();c.fill();
    c.fillStyle="rgba(255,255,255,.5)";
    c.fillRect(b.x,b.y,b.w,1.5);
    c.restore();
  }
  if(depth===0){
    /* 根节点：纵向渐变模拟微弧面光线 */
...
function drawMindCollapseControl(it,b,c){
  ...
  c.shadowColor="transparent";
  /* 背景：hover 时微亮 */
  const bgA=hover?.16:depth===0?"rgba(255,255,255,.2)":"rgba(45,95,211,.08)";
  c.fillStyle=depth===0?(hover?"rgba(255,255,255,.28)":"rgba(255,255,255,.2)"):dc(hover?"rgba(45,95,211,.14)":"rgba(45,95,211,.08)","rgba(255,255,255,.1)");
  roundRectPath(c,box.x,box.y,box.w,box.h,box.h/2);c.fill();
  c.lineWidth=1/z;c.strokeStyle=depth===0?"rgba(255,255,255,.24)":dc("rgba(45,95,211,.16)","rgba(255,255,255,.13)");c.stroke();
...
function drawAnnotation(it,b){
  ...
  if(it.detail){
    ...
    ctx.save();
    ctx.shadowColor=expanded?"rgba(45,95,211,.28)":"transparent";
    ...
    if(expanded){
      ...
      ctx.fillStyle="rgba(45,95,211,.12)";
    }
    roundRectPath(ctx,box.x,box.y,box.w,box.h,box.h/2);
    ctx.fill();
    ctx.lineWidth=1/z;ctx.strokeStyle=expanded?"rgba(255,255,255,.35)":"rgba(45,95,211,.24)";ctx.stroke();
    /* 顶部内高光 */
    if(!expanded){
...
function drawFileCard(it,cc){
  ...
  c.lineWidth=1/z;c.strokeStyle=SC.borderColor;
  ...
  if(it.previewOpen){
    ...
    c.fillText(truncateStr(f.name,b.w-72),b.x+10,b.y+headH/2);
    /* 明确标识的收起按钮：右上角圆钮 + × 图标（唯一收起热区） */
    const cb=previewCollapseBounds(it);
    if(cb){
      c.save();
      c.translate(cb.x+cb.w/2,cb.y+cb.h/2);
      c.fillStyle=dc("rgba(45,95,211,.14)","rgba(255,255,255,.16)");
      c.beginPath();c.arc(0,0,cb.w/2,0,Math.PI*2);c.fill();
      c.strokeStyle=dc("rgba(45,95,211,.5)","rgba(255,255,255,.4)");c.lineWidth=1/z;
      c.beginPath();c.arc(0,0,cb.w/2,0,Math.PI*2);c.stroke();
      c.strokeStyle=dc("#4a5a7b","#c8c8cc");c.lineWidth=1.6/z;c.lineCap="round";
      const s=cb.w*0.28;
    ...
    if(it.previewOpen){
      ...
      if(pv&&pv.kind==="img"&&pv.img){
        c.drawImage(pv.img,b.x,cy,b.w,ch);
      }else if(pv&&pv.text){
        /* 文本：真实内容预览（等宽字体 + 行距，保证可读） */
        c.fillStyle=dc("#ffffff","#1c1c1e");c.fillRect(b.x,cy,b.w,ch);
        ...
        /* 文档/表格/PPT/PDF/链接/音视频：由 DOM 覆盖层直接渲染真实预览
           （cavas 只画一个半透明白底占位，避免闪烁） */
        c.fillStyle=dc("rgba(255,255,255,.88)","rgba(30,30,34,.88)");c.fillRect(b.x,cy,b.w,ch);
        ensureMorphDom(it);
      }else{
        /* 其他二进制：信息卡 + 下载降级提示 */
        c.fillStyle=dc("#f5f6f8","#22222a");c.fillRect(b.x,cy,b.w,ch);
...
/* 完成：收回窗口 → 元素形变回归展开样式并展示内容 */
function finishMdEdit(){
  const it=detailEditId!==null?state.items.find(i=>i.id===detailEditId):null;
  if(it&&it.detail!==detailTextarea.value)pushHistory("编辑展开内容");
  if(it)it.detail=detailTextarea.value;
  closeMdModal();
  detailEditId=null;
  activeMdEditor=null;
  /* 保持元素本体为展开态（展示已添加内容），不再强制收起 */
  if(it&&detailItemId!==it.id)expandDetailInPlace(it,true);
  render();saveState();
}
document.querySelector("#detailPanel .dp-done").addEventListener("click",finishMdEdit);
document.querySelector("#detailPanel .dp-close").addEventListener("click",()=>{
  finishMdEdit();
});
/* 点击遮罩空白处也可收起（与"完成"同一套收起动画） */
if(document.getElementById("mdModalMask")){
  document.getElementById("mdModalMask").addEventListener("click",()=>{finishMdEdit();});
}
...
detailTextarea.addEventListener("keydown",e=>{
  ...
  if(mod&&(e.key==="b"||e.key==="B")){e.preventDefault();mdWrap("**","**");}
  if(mod&&(e.key==="i"||e.key==="I")){e.preventDefault();mdWrap("*","*");}
});
/* ============================================================
...
function dkBtn(opt){
  ...
  b.innerHTML=(opt.icon||"")+(opt.label?'<span>'+opt.label+'</span>':"");
...
function dockContent(st){
  ...
  if(st==="edit"){
    ...
    if(editingNoteId!==null){
      ...
      out.push(dkBtn({icon:"☑",label:"待办",title:"插入待办事项",fn:()=>mdLinePrefix("- [ ] ",true)}));
      out.push(dkBtn({icon:"▦",label:"表格",title:"插入表格",fn:()=>mdInsert("| 列 1 | 列 2 |\n| --- | --- |\n| 内容 | 内容 |")}));
      out.push(dkBtn({icon:"❝",label:"引用",title:"引用",fn:()=>mdLinePrefix("> ",true)}));
  ...
  if(st==="select"&&sel){
    ...
    if(sel.type==="mindNode"){
      out.push(dkBtn({icon:ICON.connector,label:"连接",title:"连接选中 (C)",fn:()=>toggleLink()}));
      out.push(dkBtn({icon:ICON.focus,label:"聚焦",title:"聚焦 (F)",fn:()=>{state.selected=sel.id;enterFocus(sel.id);}}));
      out.push(dkBtn({icon:ICON.plus,label:"展开",title:"展开内容 (E)",fn:()=>{state.selected=sel.id;openDetail();}}));
      out.push(dkSep());
      ...
      out.push(dkSep());
      out.push(dkBtn({icon:"✎",label:"编辑",title:"编辑内容 (双击)",fn:()=>{if(sel.type==="note")openTextEditor(sel);else{state.selected=sel.id;openDetail();}}}));
    }else{
      out.push(dkBtn({icon:ICON.focus,label:"聚焦",title:"聚焦 (F)",fn:()=>{state.selected=sel.id;enterFocus(sel.id);}}));
      out.push(dkBtn({icon:ICON.annotate,label:"批注",title:"批注 (A)",fn:()=>openAnnotation()}));
    }
  ...
  out.push(dkBtn({icon:ICON.palette,label:"背景",title:"背景样式（点击展开选择）",on:dockSubKind==="bg",fn:()=>{toggleDockSub("bg");}}));
  ...
  out.push(dkSep());
  out.push(dkBtn({icon:"☐",label:"框选",title:"批量框选",on:state.tool==="marquee",fn:()=>{setTool(state.tool==="marquee"?"select":"marquee");renderDock(true);}}));
...
function dockTargetState(){
  if(editingNoteId!==null||editingMindId!==null||detailEditId!==null)return "edit";
...
const LAYOUT_ORDER=[["right","逻辑图（向右）"],["left","逻辑图（向左）"],["org","组织图（向下）"],["u","U 型布局"],["fishbone","鱼骨图"],["timeline","时间轴"],["brace","括号图"]];
/* 背景色板（与 _BCM 一致） */
/* 背景色板（与 BK 函数内 _BCM 一致，供二级菜单全局使用） */
const BG_COLOR_MAP={default:{l:"#ffffff",d:"#1a1a2e"},eye:{l:"#c8e6c9",d:"#1a2a1e"},cream:{l:"#fff8e1",d:"#2a2a20"},blue:{l:"#e3f2fd",d:"#1a2030"},kraft:{l:"#f4ecd8",d:"#2a2218"}};
...
function applyBgColor(name){
  ...
  const c=BG_COLOR_MAP[name]||BG_COLOR_MAP.default;
  state.bgColor=state.dark?c.d:c.l;
...
function renderDockSub(){
  ...
  if(dockSubKind==="layout"){
    ...
    for(const [id,label] of BG_COLORS){
      ...
      const col=(BG_COLOR_MAP[id]||BG_COLOR_MAP.default)[state.dark?"d":"l"];
...
/* ============================================================
   展开内容：Markdown 工具栏
============================================================ */
const dpPreview=document.querySelector("#detailPanel .dp-md-preview");
/* 统一 MD 编辑：当前活跃编辑器（便签浮动态 / 展开居中态共用同一套操作逻辑） */
let activeMdEditor=null;
function mdTarget(){return activeMdEditor||detailTextarea;}
...
document.querySelectorAll("#detailPanel .dp-t").forEach(btn=>{
  btn.addEventListener("click",()=>{
    const act=btn.dataset.act;
    if(act==="bold")mdWrap("**","**","加粗");
    else if(act==="italic")mdWrap("*","*","斜体");
    else if(act==="strike")mdWrap("~~","~~","删除线");
    else if(act==="code")mdWrap("`","`","code");
    else if(act==="ul")mdLinePrefix("- ",true);
    else if(act==="ol")mdLinePrefix("1. ",true);
    else if(act==="todo")mdLinePrefix("- [ ] ",true);
    else if(act==="quote")mdLinePrefix("> ",true);
    else if(act==="table")mdInsert(TABLE_TPL);
    else if(act==="hr")mdInsert("\n---\n");
    else if(act==="link")mdInsert("[链接文字](https://)");
    else if(act==="preview"){
      const on=dpPreview.classList.toggle("show");
      btn.classList.toggle("on",on);
      detailTextarea.style.display=on?"none":"";
      if(on)renderDpPreview();
    }
  });
});
document.querySelector("#detailPanel .dp-sel").addEventListener("change",e=>{
  const v=e.target.value;
  if(v)mdLinePrefix(v,false);
  e.target.value="";
});
/* 预览渲染：Markdown → HTML（含待办/表格） */
function renderDpPreview(){
  if(!dpPreview)return;
  dpPreview.innerHTML=mdToHtml(detailTextarea.value);
  /* 待办可勾选（仅预览态交互，不写回文本） */
  dpPreview.querySelectorAll('input[type="checkbox"]').forEach(cb=>{
    cb.addEventListener("change",()=>{cb.parentElement.style.opacity=cb.checked?.55:1;});
  });
}
function mdToHtml(src){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(t===""){if(inList){out+=inList==="ul"?"</ul>":"</ol>";inList=null;}out+="";continue;}
    /* 标题 */
    const h=t.match(/^(#{1,4})\s+(.*)$/);
    if(h){const lv=h[1].length;out+="<h"+lv+">"+inline(h[2])+"</h"+lv+">";continue;}
    /* 分隔线 */
    if(/^(-{3,}|\*{3,})$/.test(t)){out+="<hr>";continue;}
    /* 引用 */
    if(/^>\s?/.test(t)){out+="<blockquote>"+inline(t.replace(/^>\s?/,""))+"</blockquote>";continue;}
    /* 待办 */
    const todo=t.match(/^[-*]\s+\[([ xX])\]\s+(.*)$/);
    if(todo){
      const ck=todo[1].toLowerCase()==="x";
      if(inList){out+=inList==="ul"?"</ul>":"</ol>";inList=null;}
      out+='<li class="task"><input type="checkbox"'+(ck?" checked":"")+"><span>"+inline(todo[2])+"</span></li>";
      continue;
    }
    /* 列表 */
    const li=t.match(/^[-*]\s+(.*)$/);
    if(li){
      if(inList!=="ul"){if(inList)out+="</ol>";out+="<ul>";inList="ul";}
      out+="<li>"+inline(li[1])+"</li>";continue;
    }
    const oli=t.match(/^\d+\.\s+(.*)$/);
    if(oli){
      if(inList!=="ol"){if(inList)out+="</ul>";out+="<ol>";inList="ol";}
      out+="<li>"+inline(oli[1])+"</li>";continue;
    }
    out+="<p>"+inline(t)+"</p>";
  }
  if(inCode)out+="</code></pre>";
  if(inTable)out+="</tbody></table>";
  if(inList)out+=inList==="ul"?"</ul>":"</ol>";
  return out;
  function inline(s){
    let x=esc(s);
...
/* 编辑时同步刷新预览（预览态开启时） */
detailTextarea.addEventListener("input",()=>{
  if(dpPreview&&dpPreview.classList.contains("show"))renderDpPreview();
});
/* 批注：A键，选中元素后添加常驻小字 */
...
function openDetail(){
  ...
  detailEditId=s.id;
  activeMdEditor=detailTextarea;
  detailTextarea.value=s.detail||"";
  /* 展开编辑：居中大窗 + 背景模糊（明确暗示"正在展开编辑"） */
  openMdModal();
  detailTextarea.focus();
  render();
}
/* 统一的 MD 浮层：展开态（居中大窗 + 模糊背景） */
const mdMask=document.getElementById("mdModalMask");
function openMdModal(){
  if(!mdMask)return;
  mdMask.classList.remove("closing");
  mdMask.classList.add("show");
  detailPanel.classList.add("modal");
  /* 下一帧加 show，触发过渡动画（scale .9→1 + 淡入） */
  requestAnimationFrame(()=>{detailPanel.classList.add("show");});
}
function closeMdModal(){
  if(!mdMask)return;
  mdMask.classList.add("closing");
  detailPanel.classList.remove("show");
  /* 等动画结束再彻底隐藏（连贯收起） */
  setTimeout(()=>{
    mdMask.classList.remove("show","closing");
    detailPanel.classList.remove("modal");
  },240);
}
...
function detailContentHeight(it,width){
  return clamp(26+detailRows(it,width).length*21,56,196);
}
...
function drawDetail(it,b){
  ...
  if(bodyH<4)return;
  const padX=14/z,padY=12/z,lineH=21/z;
  const lines=detailRows(it,b.w);
  const dx=b.x,dy=b.y+baseH,dw=b.w,dh=bodyH;
  ctx.save();
  const reveal=clamp(bodyH/(20/z),0,1);
  ctx.globalAlpha=reveal;
  /* 详情区背景 — 底部弹出渐变为统一底色（与元素融合，非割裂面板） */
  const dbg=ctx.createLinearGradient(dx,dy,dx,dy+dh);
  dbg.addColorStop(0,dc("rgba(45,95,211,.10)","rgba(120,170,255,.18)"));
  dbg.addColorStop(1,dc("rgba(45,95,211,.04)","rgba(120,170,255,.08)"));
  ctx.fillStyle=dbg;ctx.fillRect(dx,dy,dw,dh);
  /* 顶部柔光分隔线（渐变，从边缘到中央渐隐） */
  const sepG=ctx.createLinearGradient(dx,dy,dx+dw,dy);
  sepG.addColorStop(0,"rgba(255,255,255,0)");
  sepG.addColorStop(.5,dc("rgba(45,95,211,.22)","rgba(130,180,255,.3)"));
  sepG.addColorStop(1,"rgba(255,255,255,0)");
  ctx.strokeStyle=sepG;ctx.lineWidth=1/z;
  ...
  for(const ln of lines){
    const tx=dx+padX+(ln.indent?14/z:0);
    ctx.font=detailFont(ln);
    ctx.fillStyle=ln.h>0?dc("#1d1d1f","#f5f5f7"):ln.quote?dc("#8a8a90","#7c7c82"):dc("#4a4a4f","#c8c8cc");
...
function drawLinks(scope){
  ...
  for(const l of state.links){
    ...
    /* 连线透明度/粗细随风格变化，强化风格差异 */
    const LK=styleCfg();
    ctx.globalAlpha=active?.9:LK.linkAlpha;
    /* 线型编码：关联/支撑=实线，导致=粗实线，反证=虚线，证据=点划线 */
    const baseW=(l.relationType==="causes"?3.2:2.5)*(LK.linkWidth||1);
    ...
    /* 通用路线：拟物纸感走钢笔笔触，其余走方向感知的 S 形/圆角折线 */
    if(LK.linkStyle==="pen") strokeLinkCurve(ctx,curve);
    else routeConnection(ctx,ea,eb,curve.outSide,curve.inpSide,linkColor,baseW/z);
    ctx.stroke();ctx.setLineDash([]);
    /* 丝质高光 — 关系线叠加极细白光 */
...
function logicLeftLayout(){
  ...
  function place(node,xRight,cursor){
    ...
    const b=itemBounds(node),ks=kidsOf(node);
    const total=subTreeH(node,GAP_Y);
    node.x=xRight-b.w;node.y=cursor+total/2-b.h/2;
    if(!ks.length)return;
    let cy=cursor;
    for(const k of ks){
      const kh=subTreeH(k,GAP_Y);
...
function uShapeLayout(){
  ...
  for(const k of l1){
    ...
    cy=Math.max(rowY,cy+kb.h+GAP_Y);
  }
}
/* ---------------- 2. 组织结构图（向下） ---------------- */
function orgDownLayout(){
  const root=layoutRoot();if(!root)return;
  const GAP_Y=64,GAP_X=26;
  const rb=itemBounds(root);root.x=-rb.w/2;root.y=-rb.h/2;
  function subTreeW(n){
    const b=itemBounds(n),ks=kidsOf(n);
...
/* ③ 排序：同级按名称稳定排序（中文用 localeCompare，数字优先） */
function sortSiblings(){
  const key=n=>String(n.text||"");
  for(const n of state.items){
    ...
    kids.sort((a,b)=>key(a).localeCompare(key(b),"zh-Hans-CN",{numeric:true}));
    n.children=kids.map(k=>k.id);
  }
}
/* ⑤ 归位附属：把便签/附件放到离它最近的节点旁边（不再原地不动） */
  ...
  for(let pass=0;pass<3;pass++){
    ...
    for(let i=0;i<attach.length;i++){
      for(let j=i+1;j<attach.length;j++){
        const A=itemBounds(attach[i]),B=itemBounds(attach[j]);
        if(!A||!B)continue;
  ...
  for(let pass=0;pass<3;pass++){
    ...
    for(let i=0;i<elems.length;i++){
      for(let j=i+1;j<elems.length;j++){
        ...
        const ba=itemBounds(a),bb=itemBounds(b);
        /* 检查重叠 */
...
/* （已移除重复的 openTextEditor 定义，保留后方的唯一实现） */
...
const SEL_LABEL={note:"便签",stroke:"画笔",connector:"连线",mindNode:"导图节点",fileCard:"文件"};
...
function renderPvContent(pv,f,body){
  ...
  const kick=(url,type)=>{
    if(type==="img"){
      ...
      /* iframe 舞台：固定逻辑视口，整体由 transform 做等比缩放（与画布 zoom 实时同步）
         而不是用 width:100% 让内容按放大后的容器重排——那是"内容不随缩放"的根因 */
      const stage=document.createElement("div");
      stage.className="pv-stage";
      stage.style.cssText="flex:1;overflow:hidden;display:flex;justify-content:center;align-items:flex-start;background:#f6f7f9";
      ...
      /* 逻辑视口基准：PC 1280×800 / 手机 375×667（世界坐标 px） */
      const LOGICAL_W=1280,LOGICAL_H=800,MB_W=375,MB_H=667;
      let mbMode=(f.kind==="link");   /* 网页链接默认手机模式（375 逻辑宽），保证宽度内全部内容可见 */
      /* 挂引用到 body，供 syncPvContentZoom 读取 */
      body.__pvStage=stage;body.__pvIframe=iframe;body.__pvMb=()=>mbMode;
      const layout=()=>{
        iframe.style.cssText="border:none;width:"+(mbMode?MB_W:LOGICAL_W)+"px;height:"+(mbMode?MB_H:LOGICAL_H)+"px;flex:none;transform-origin:top center";
      };
      const setPC=()=>{mbMode=false;layout();pcBtn.style.background="var(--accent)";pcBtn.style.color="#fff";mbBtn.style.background="transparent";mbBtn.style.color="var(--ink-dim)";};
      const setMB=()=>{mbMode=true;layout();mbBtn.style.background="var(--accent)";mbBtn.style.color="#fff";pcBtn.style.background="transparent";pcBtn.style.color="var(--ink-dim)";};
      layout();
      if(mbMode)setMB();else setPC();
...
============================================================ */
let morphDom=null;                 /* 单例覆盖层 */
let morphDomFileId=null;
let morphToken=0;                  /* 令牌：异步回调过期判定，防竞态写 detached DOM */
function ensureMorphDom(it){
  if(!it||it.type!=="fileCard")return;
  if(morphDomFileId===it.fileId)return;   /* 已渲染同文件 */
  destroyMorphDom();
  const f=state.files.find(x=>x.id===it.fileId);
  if(!f)return;
  const layer=document.getElementById("previewLayer");
  if(!layer)return;
  morphDom=document.createElement("div");
  morphDom.className="pv-morph";
  morphDom.dataset.card=String(it.id);   /* dataset 恒为字符串，必须 String 化比对的基准 */
  layer.appendChild(morphDom);
  morphDomFileId=it.fileId;
  const token=++morphToken;
  /* 过期检查：若此回调执行时已被新预览取代/关闭，则丢弃 */
  const stale=()=>!morphDom||morphDom.dataset.card!==String(it.id)||token!==morphToken;
  const body=morphDom;
  body.innerHTML='<div class="pv-loading"><div class="spinner"></div>加载中…</div>';
  if(f.kind==="link"){
    if(!isSafePreviewUrl(f.url)){body.innerHTML='<div class="pv-msg"><span class="big">⚠️</span>链接地址无效，未加载预览</div>';return;}
    kickMorph(f.url,"link");
  }else{
    getBlob(it.fileId).then(b=>{
      ...
      try{
        ...
        else if(f.kind==="media")kickMorph(f._url,"media");
        else if(f.kind==="text"||f.kind==="code"||/\.(md|txt|csv|json|log)$/i.test(f.name)){
          /* 文本：直接用 blob 转字符串（比 fetch(blobURL) 更稳，兼容打包后 file:// 协议） */
          kickTextBlob(b,it.id);
        }
        else kickMorph(f._url,f.kind);
      }catch(e){
        if(stale())return;
...
/* 形变覆盖层的渲染（复用 renderPvContent 的 kick 逻辑，绑定 morphDom） */
/* 形变覆盖层的渲染（复用 renderPvContent 的 kick 逻辑，绑定 morphDom） */
function kickTextBlob(blob,cardId){
  if(!morphDom||morphDom.dataset.card!==String(cardId))return;
  const token=morphToken,body=morphDom;
  blob.text().then(t=>{
    if(morphToken!==token)return;
    body.innerHTML='<pre class="pv-text"></pre>';
    body.firstChild.textContent=(t||"").slice(0,20000);
  }).catch(()=>{if(morphToken===token)body.innerHTML='<div class="pv-msg">无法读取文本</div>';});
}
function kickMorph(url,type){
  if(!morphDom)return;
  const token=morphToken;
  const body=morphDom;
  if(type==="img"){
    body.innerHTML='<div class="morph-fit"><img src="'+url+'" style="max-width:100%;max-height:100%;object-fit:contain;margin:auto"></div>';
  }else if(type==="media"){
    const v=document.createElement("video");
    v.style.cssText="width:100%;height:100%;object-fit:contain;background:#000";
    v.src=url;v.controls=true;v.autoplay=false;
    v.onerror=()=>{if(morphToken===token)body.innerHTML='<div class="pv-msg"><span class="big">⚠️</span>视频加载失败<br><a download>下载后本地播放</a></div>';};
    body.innerHTML="";body.appendChild(v);
  }else if(type==="text"){
    ...
    iframe.src=url;
    iframe.sandbox="allow-forms allow-scripts allow-popups";
    iframe.referrerPolicy="no-referrer";
    iframe.title="网页材料预览";
    /* 固定逻辑宽 + transform scale，随画布 zoom 同步（与预览窗策略一致）
       网页链接默认手机模式 375 宽，保证宽度内全部内容可见 */
    let mbMode=true;
    const LW=375, LH=667;
    iframe.style.cssText="border:none;width:"+LW+"px;height:"+LH+"px;flex:none;transform-origin:top center";
    iframe.onerror=()=>{body.innerHTML='<div class="pv-msg"><span class="big">⚠️</span>链接加载失败<br><a href="'+url+'" target="_blank" rel="noopener">在新窗口打开</a></div>';};
    stage.appendChild(iframe);
    wrap.appendChild(stage);
    body.innerHTML="";body.appendChild(wrap);
    /* 供 syncMorphDom 同步缩放（__pvMb 返回当前手机/PC 模式） */
    body.__pvIframe=iframe;body.__pvStage=stage;body.__pvMb=()=>mbMode;
    syncMorphDom();
  }else{
...
function destroyMorphDom(){
  morphToken++;   /* 令牌递增：所有在途异步回调立即失效 */
  if(morphDom){morphDom.remove();morphDom=null;}
  morphDomFileId=null;
}
async function ensureCdn(kind){
  ...
  /* 本地优先：打包桌面应用时，将 vendor 脚本放入 assets/vendor/ 下，
     与 HTML 相对路径引用，离线也能渲染。找不到本地文件时自动回退 CDN。 */
  const LOCAL=function(key2){
    const names={jszip:"jszip.min.js",docx:"docx-preview.min.js",xlsx:"xlsx.full.min.js",pdfjs:"pdf.min.js",pdfjsWorker:"pdf.worker.min.js"};
    return "assets/vendor/"+names[key2];
  };
  ...
  try{
    ...
    if(key==="pdfjs"){
      ...
      if(window.pdfjsLib){
        let okW=false;
        try{await loadScript(LOCAL("pdfjsWorker"));okW=true;}catch(e){}
        if(!okW){window.pdfjsLib.GlobalWorkerOptions.workerSrc=CDN.pdfjsWorker.url;}
      }
...
async function renderDocx(b,bodyEl){
  ...
  if(!ok){bodyEl.innerHTML='<div class="pv-msg"><span class="big">⚠️</span>加载 Word 预览组件失败<br>请检查网络连接或放入 assets/vendor/docx-preview.min.js<br><a download>下载文件本地打开</a></div>';return;}
...
function saveState(){
  try{
    const data={
      ...
      ui:{sideCollapsed:state.sideCollapsed,dark:state.dark,bgPattern:state.bgPattern,bgColorName:state.bgColorName,mindMode:state.mindMode,layoutType:state.layoutType,fontPreset:state.fontPreset,stylePreset:state.stylePreset},
      savedAt:Date.now(),
    };
...
function loadState(){
  try{
    ...
    if(d.ui){
      ...
      if(d.ui.stylePreset&&STYLE_PRESETS[d.ui.stylePreset])state.stylePreset=d.ui.stylePreset;
...
function mountControls(){
  ...
  /* 收起态恢复按钮：点击恢复侧栏固定（同时退出自动折叠标记，避免响应式自动改回） */
  const sideRestore=document.getElementById("side-restore");
  if(sideRestore)sideRestore.addEventListener("click",()=>{
    state.sideCollapsed=false;state._respAuto=false;
    applySide();saveState();
  });
  ...
  const MENUS={
    layout:[
      ...
      {icon:"",label:"逻辑图（向左）",key:"",fn:()=>{state.layoutType="left";autoLayout();render();saveState();toast("逻辑图（向左）");}},
      {icon:"",label:"组织结构图（向下）",key:"",fn:()=>{state.layoutType="org";autoLayout();render();saveState();toast("组织结构图（向下）");}},
      {icon:"",label:"U 型布局",key:"",fn:()=>{state.layoutType="u";autoLayout();render();saveState();toast("U 型布局");}},
      ...
      {sep:true},
      {icon:"",label:"样式：清晰",key:"",fn:()=>{state.stylePreset="clear";render();saveState();toast("样式：清晰");}},
      {icon:"",label:"样式：玻璃拟态",key:"",fn:()=>{state.stylePreset="glass";render();saveState();toast("样式：玻璃拟态（半透明）");}},
      {icon:"",label:"样式：拟物纸感",key:"",fn:()=>{state.stylePreset="paper";render();saveState();toast("样式：拟物纸感（钢笔连线）");}},
      {icon:"",label:"样式：新拟态",key:"",fn:()=>{state.stylePreset="soft";render();saveState();toast("样式：新拟态");}},
    ],
    add:[
...
function applySide(){
  ...
  /* 收起态：显示恢复按钮（常驻入口，不随 peek 消失）；常驻态隐藏 */
  const restoreBtn=document.getElementById("side-restore");
  if(restoreBtn){
    restoreBtn.classList.toggle("show",!!state.sideCollapsed);
    restoreBtn.innerHTML=ICON.expand;  /* 右指向：点击恢复 */
  }
  /* 热区：仅收起态启用；收起时确保清掉浮出态 */
  const hz=document.getElementById("side-hotzone");
  if(hz){
    hz.classList.toggle("active",!!state.sideCollapsed);
    if(!state.sideCollapsed){sidePanel.classList.remove("peek");if(peekTimer){clearTimeout(peekTimer);peekTimer=0;}}
  }
...
/* ============================================================
   ...
   —— 吸附：浮出时左边缘严格贴 0；弹回后完全移出屏幕（translateX(-100%)），
             不残留任何白框/白条
============================================================ */
let peekTimer=0,peekLocked=false;
const PEEK_DELAY=300;   /* 离开后滞后收回应答阈值（避免轻微右移即收回） */
function sidePeek(on){
  ...
  if(on){sidePanel.classList.add("peek");return;}
...
/* 指针是否悬停在左边缘热区/侧栏区域（含弹层）内 */
function isPointerOverHoverZone(){
  /* 用一个实时记录的 lastPointer（由全局 pointermove 更新）判断 */
  if(!lastPointer)return false;
  const hz=document.getElementById("side-hotzone");
  if(hz){
    const r=hz.getBoundingClientRect();
    if(lastPointer.x>=r.left&&lastPointer.x<=r.right&&lastPointer.y>=r.top&&lastPointer.y<=r.bottom)return true;
  }
...
(function initSidePeek(){
  const hz=document.getElementById("side-hotzone");
  if(!hz)return;
  /* 进入热区 → 浮出（并清掉正在进行的收回计时，形成缓冲） */
  hz.addEventListener("mouseenter",()=>sidePeek(true));
  /* 鼠标移入面板 → 保持浮出（锁定，防止移开即刻收起） */
  sidePanel.addEventListener("mouseenter",()=>{peekLocked=true;sidePeek(true);});
  /* 鼠标离开面板 → 解除锁定并延迟弹回（300ms 滞后） */
  sidePanel.addEventListener("mouseleave",()=>{peekLocked=false;sidePeek(false);});
  /* 键盘可达：Tab 聚焦到面板内也浮出 */
  sidePanel.addEventListener("focusin",()=>{peekLocked=true;sidePeek(true);});
  /* 热区离开：同样走滞后缓冲（不再立即收回） */
  hz.addEventListener("mouseleave",()=>{
    /* 鼠标可能刚越过热区进入面板（mouseenter 已触发），无需处理；
       若确实离开到画布，由全局 pointermove 兜底 */
  });
